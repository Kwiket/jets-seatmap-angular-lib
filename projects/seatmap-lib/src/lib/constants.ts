import { TSeatType } from './types';

export const ENTITY_TYPE_MAP = {
  seat: 'seat' as const,
  aisle: 'aisle' as const,
  empty: 'empty' as const,
  index: 'index' as const,
};

export const ENTITY_STATUS_MAP = {
  available: 'available' as const,
  unavailable: 'unavailable' as const,
  selected: 'selected' as const,
  preferred: 'preferred' as const,
  extra: 'extra' as const,
  disabled: 'disabled' as const,
};

/** Maps seatScheme chars to seat types (legacy format) */
export const SEAT_SCHEME_MAP: Record<string, TSeatType> = {
  S: 'seat',
  E: 'empty',
  '-': 'aisle',
};

/** Maps API numeric seat type to internal type */
export const API_SEAT_TYPE_MAP: Record<number, TSeatType> = {
  0: 'seat',
  1: 'aisle',
  2: 'empty',
  3: 'seat', // seat with restrictions
};

export const CLASS_CODE_MAP: Record<string, string> = {
  f: 'First',
  b: 'Business',
  p: 'Premium economy',
  e: 'Economy',
  a: 'All',
  F: 'First',
  B: 'Business',
  P: 'Premium economy',
  E: 'Economy',
  A: 'All',
};

export const DEFAULT_LANG = 'EN';
export const DEFAULT_UNITS = 'metric';
export const DEFAULT_SEAT_MARGIN = 2;
export const DEFAULT_SEAT_MAP_WIDTH = 350;
export const DEFAULT_SEAT_SIZE = 32;
export const DEFAULT_TOOLTIP_WIDTH = 300;
export const DEFAULT_AUTHORIZATION_SCHEME = 'Bearer';

export const SCALE_TYPES = {
  ZOOM: 'zoom' as const,
  SCALE: 'scale' as const,
};

export const DEFAULT_SCALE_TYPE = SCALE_TYPES.SCALE;

/** Default seat type (#3 — regular economy seat), used as fallback when seat type is unknown. */
export const DEFAULT_SEAT_TYPE = 3;

/** Per-seat-type pixel dimensions [width, height] — 50 entries, indexed by seatType number. */
export const SEAT_SIZE_BY_TYPE: [number, number][] = [
  [100, 100],
  [122, 218],
  [150, 170],
  [100, 100],
  [122, 200],
  [200, 200],
  [200, 200],
  [140, 200],
  [150, 200],
  [150, 200],
  [200, 150],
  [200, 150],
  [200, 185],
  [200, 185],
  [120, 150],
  [150, 175],
  [150, 175],
  [185, 175],
  [185, 175],
  [255, 175],
  [130, 330],
  [130, 330],
  [200, 400],
  [200, 400],
  [200, 365],
  [200, 365],
  [200, 200],
  [200, 200],
  [200, 200],
  [200, 200],
  [200, 265],
  [200, 265],
  [200, 330],
  [200, 330],
  [200, 200],
  [200, 200],
  [550, 435],
  [550, 435],
  [550, 435],
  [550, 435],
  [275, 1230],
  [275, 1230],
  [550, 325],
  [550, 325],
  [550, 325],
  [550, 325],
  [100, 100],
  [200, 200],
  [200, 200],
  [100, 100],
];

// Default color theme — WCAG 2.1/2.2 AA compliant defaults.
//
// All colour pairs that carry information were audited against WCAG SC 1.4.3
// (text contrast, minimum 4.5:1) and SC 1.4.11 (non-text / UI boundary
// contrast, minimum 3:1). Contrast values below were computed with the
// standard relative-luminance formula (https://www.w3.org/TR/WCAG21/#dfn-relative-luminance)
// using opaque hex values — alpha colours are deliberately avoided because
// effective contrast then depends on the host page background and cannot be
// certified.
//
// Verified pairs (label on fill — text, SC 1.4.3, ≥ 4.5:1):
//   seatLabelColor          #1a1a1a on seatAvailableColor      #A5D6A7 → 10.59:1 ✓
//   seatLabelColor          #1a1a1a on seatSelectedColor       #90CAF9 →  9.95:1 ✓
//   seatLabelColor          #1a1a1a on seatPreferredColor      #FFCC80 → 11.77:1 ✓
//   seatLabelColor          #1a1a1a on seatExtraColor          #CE93D8 →  7.28:1 ✓
//   seatLabelColor          #1a1a1a on notAvailableSeatsColor  #BDBDBD →  9.26:1 ✓
//   seatLabelColor          #1a1a1a on seatUnavailableColor    #BDBDBD →  9.26:1 ✓
//
// Verified pairs (UI boundary / status indicator, SC 1.4.11, ≥ 3:1):
//   seatStrokeColor         #757575 on seatMapBackgroundColor  #ffffff →  4.61:1 ✓
//
// Verified pairs (tooltip text + buttons, SC 1.4.3, ≥ 4.5:1):
//   tooltipFontColor        #333333 on tooltipBackgroundColor          #ffffff → 12.63:1 ✓
//   tooltipSelectButtonTextColor #ffffff on tooltipSelectButtonBackgroundColor rgb(0,68,153) →  9.18:1 ✓
//   tooltipCancelButtonTextColor #333333 on tooltipCancelButtonBackgroundColor #f0f0f0       → 11.09:1 ✓
//   defaultPassengerBadgeLabelColor #ffffff on defaultPassengerBadgeColor #1565C0            →  6.73:1 ✓
//
// Verified pairs (incidental text on background):
//   cabinTitlesLabelColor   #0277BD on seatMapBackgroundColor  #ffffff →  4.80:1 ✓
//
// ⚠ When updating ANY colour above, recompute the affected pairs and refresh
// the table above. Consumers who pass a custom `colorTheme` are unaffected
// and own their own contrast budget.
export const DEFAULT_COLOR_THEME = {
  // Seat
  seatAvailableColor: '#A5D6A7',
  seatUnavailableColor: '#BDBDBD',
  seatSelectedColor: '#90CAF9',
  seatPreferredColor: '#FFCC80',
  seatExtraColor: '#CE93D8',
  seatLabelColor: '#1a1a1a',
  seatStrokeColor: '#757575',
  seatStrokeWidth: 1,
  seatArmrestColor: 'rgb(185,186,186)',
  notAvailableSeatsColor: '#BDBDBD',
  // Fuselage
  fuselageFillColor: 'lightgray',
  fuselageStrokeColor: 'darkgrey',
  fuselageStrokeWidth: 12,
  fuselageWindowsColor: 'darkgrey',
  fuselageWingsColor: 'darkgrey',
  fuselageNoseType: 'default' as 'default' | 'by-type',
  // Floor & background
  seatMapBackgroundColor: '#ffffff',
  floorColor: '#595959',
  seatmapFontColor: '#333333',
  // Bulk
  bulkIconColor: 'rgb(70, 81, 94)',
  bulkFloorIconColor: 'rgb(206, 216, 237)',
  bulkBaseColor: 'dimgrey',
  bulkCutColor: 'lightgrey',
  // Armrest / exit / hull
  armrestColor: 'rgb(185,186,186)',
  exitColor: '#d00434',
  hullColor: '#e8eaed',
  // Passenger badge
  defaultPassengerBadgeColor: '#1565C0',
  defaultPassengerBadgeLabelColor: '#ffffff',
  defaultPassengerBadgeBorderColor: '#0D47A1',
  // Tooltip
  tooltipBackgroundColor: '#ffffff',
  tooltipHeaderColor: '#222222',
  tooltipBorderColor: '#e0e0e0',
  tooltipFontColor: '#333333',
  tooltipIconColor: '#4f6f8f',
  tooltipIconBorderColor: '#4f6f8f',
  tooltipIconBackgroundColor: '#ffffff',
  tooltipSelectButtonTextColor: '#ffffff',
  tooltipSelectButtonBackgroundColor: 'rgb(0, 68, 153)',
  tooltipCancelButtonTextColor: '#333333',
  tooltipCancelButtonBackgroundColor: '#f0f0f0',
  // Deck
  deckTitleColor: '#333333',
  deckLabelTitleColor: 'rgb(255,255,255)',
  deckHeightSpacing: 200,
  deckSeparation: 50,
  // Deck selector — matches React defaults from constants.js
  deckSelectorStrokeColor: 'rgba(50, 50, 50, 0.5)',
  deckSelectorFillColor: '#fff',
  deckSelectorSize: 25,
  // Wings (React default: 30)
  wingsWidth: 30,
  // Cabin titles
  cabinTitlesWidth: 80,
  cabinTitlesLabelColor: '#0277BD',
  cabinTitlesHighlightColors: { F: '#BDB76B', B: '#FF8C00', P: '#8FBC8F', E: '#1E90FF' } as Record<string, string>,
  // Typography
  fontFamily: '',
};

// ─── Localisations ────────────────────────────────────────────────────────────
// React parity: shared keys mirror jets-seatmap-react-lib-pub/src/common/i18n.languages.js
// (regenerated via .claude/jobs/2f528823/merge-locales.mjs). Angular-only keys
// (status labels, legend, gallery, deprecated power/USB synonyms) are preserved.

export const LOCALIZATION_EN: Record<string, string> = {
  deck: 'Deck',
  cancel: 'Cancel',
  select: 'Select',
  unselect: 'Unselect',
  available: 'Available',
  unavailable: 'Unavailable',
  selected: 'Selected',
  preferred: 'Preferred',
  extra: 'Extra',
  disabled: 'Disabled',
  pitch: 'Pitch',
  width: 'Width',
  recline: 'Recline',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi enabled',
  wifiEnabled: 'Wi-Fi enabled',
  power: 'Power plug',
  powerPlug: 'Power plug',
  usbPlug: 'USB plug',
  usbPowerPlug: 'USB and power plug',
  powerAcUsb: 'USB and power plug',
  powerOutletOnly: 'Power plug',
  usb: 'USB plug',
  usbOnly: 'USB plug',
  audioVideo: 'Audio and video on demand',
  audio_video_ondemand: 'Audio and video on demand',
  entertainment: 'Audio and video on demand',
  bluetooth: 'Pair your headset',
  restricted_legroom: 'Constrained Legroom',
  restrictedLegroom: 'Constrained Legroom',
  extra_legroom: 'Extra legroom',
  extraLegroom: 'Extra legroom',
  noData: 'No seat map available',
  loading: 'Loading seat map...',
  first: 'First class',
  business: 'Business class',
  premium: 'Premium class',
  economy: 'Economy class',
  F: 'First class',
  B: 'Business class',
  P: 'Premium class',
  E: 'Economy class',
  babyBassinet: 'Baby bassinet available',
  bassinet: 'Baby bassinet available',
  doNotRecline: 'Restricted recline, backaches possible',
  limitedRecline: 'Restricted recline, backaches possible',
  nearGalley: 'Close to galleys',
  nearLavatory: 'Close to restrooms',
  nearStairs: 'Stairs, heavy traffic area',
  noFloorStorage: 'No underseat storage',
  noOverheadStorage: 'Limited storage space',
  limitedOverheadStorage: 'Limited storage space',
  getColdByExit: 'Close to exit, drafts and chilly',
  misalignedWindow: 'Partial or missing window view',
  noWindow: 'Partial or missing window view',
  wingInWindow: 'Wing from window view',
  trayTableInArmrest: 'Tray table in the armrest',
  storageBoxBetweenWall: 'Storage box between wall',
  standardSeat: 'Standard seat',
  reservedCrewSeat: 'Reserved',
  prereclinedSeat: 'Pre-reclined seat',
  personalTvInArmrest: 'Personal TV in armrest',
  invertedSeat: 'Rear-facing seat',
  narrower: 'Narrower seat',
  exitRow: 'Exit row',
  ADT: 'adults',
  CHD: 'children',
  INF: 'infants',
  seatRestrictions: 'The seat is only for',
  passenger: 'Passenger',
  passengers: 'Passengers',
  legend: 'Legend',
  free: 'free',
  gallery: 'Aircraft gallery',
  allCabins: 'All cabins',
  seatsIn: 'Seats in',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'window',
  seatPositionAisle: 'aisle',
  seatPositionMiddle: 'middle',
  seatExtraLegroom: 'extra legroom',
  seatExitRow: 'exit row',
  seatAvailable: 'available',
  seatUnavailable: 'unavailable',
  seatSelected: 'selected',
  seatSelectedFor: 'selected for',
  seatRestrictedFor: 'not available for',
  close: 'Close',
  moveToSeat: 'Move to seat',
  gridLabel: 'Seat map',
  allSeats: 'All seats',
  row: 'Row',
  seat: 'Seat',
  cabin: 'Cabin',
  position: 'Position',
  features: 'Features',
  price: 'Price',
  status: 'Status',
  action: 'Action',
};

export const LOCALIZATION_RU: Record<string, string> = {
  deck: 'Этаж',
  cancel: 'Закрыть',
  select: 'Выбрать',
  unselect: 'Убрать',
  available: 'Доступно',
  unavailable: 'Недоступно',
  selected: 'Выбрано',
  preferred: 'Предпочтительно',
  extra: 'Дополнительно',
  disabled: 'Отключено',
  pitch: 'Длинна',
  width: 'Ширина',
  recline: 'Наклон',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Наличие Wi-Fi',
  wifiEnabled: 'Наличие Wi-Fi',
  power: 'Розетка',
  powerPlug: 'Розетка',
  usbPlug: 'Наличие USB-порта',
  usbPowerPlug: 'Наличие розетки и USB-порта',
  audioVideo: 'Развлекательная аудио- и видеопрограмма',
  audio_video_ondemand: 'Развлекательная аудио- и видеопрограмма',
  entertainment: 'Развлекательная аудио- и видеопрограмма',
  bluetooth: 'Сопряжение гарнитуры',
  restricted_legroom: 'Мало места для ног',
  restrictedLegroom: 'Мало места для ног',
  extra_legroom: 'Много места для ног',
  extraLegroom: 'Много места для ног',
  noData: 'Карта мест недоступна',
  loading: 'Загрузка карты мест...',
  first: 'Первый класс',
  business: 'Бизнес-класс',
  premium: 'Премиум-класс',
  economy: 'Эконом-класс',
  F: 'Первый класс',
  B: 'Бизнес-класс',
  P: 'Премиум-класс',
  E: 'Эконом-класс',
  babyBassinet: 'Рядом с детской кроваткой',
  bassinet: 'Рядом с детской кроваткий',
  doNotRecline: 'Спинка кресла не отклоняется',
  limitedRecline: 'Ограниченное отклонение спинки кресла',
  nearGalley: 'Около буфета-кухни',
  nearLavatory: 'Около туалета',
  nearStairs: 'Рядом с лестницей, активное движение',
  noFloorStorage: 'Нет места для ручной клади под сиденьем',
  noOverheadStorage: 'Нет полки для ручной клади',
  limitedOverheadStorage: 'Полка ручной клади ограничена в размерах',
  getColdByExit: 'Возле выхода',
  misalignedWindow: 'Иллюминатор смещён',
  noWindow: 'Иллюминатор отстутсвует',
  wingInWindow: 'Место возле крыла самолёта',
  trayTableInArmrest: 'Откидной столик в подлокотнике',
  storageBoxBetweenWall: 'Ящик для хранения в стене',
  standardSeat: 'Стандартное сиденье',
  reservedCrewSeat: 'Зарезервированное персоналом',
  prereclinedSeat: 'Предварительно откинутое сиденье',
  personalTvInArmrest: 'Персональный телевизор в подлокотнике',
  invertedSeat: 'Сиденье устанавливаемое лицом назад',
  narrower: 'Более узкое сиденье',
  exitRow: 'Ряд возле выхода',
  ADT: 'взрослые',
  CHD: 'дети',
  INF: 'младенцы',
  seatRestrictions: 'Сиденье только для',
  passenger: 'Пассажир',
  passengers: 'Пассажиры',
  legend: 'Легенда',
  free: 'бесплатно',
  gallery: 'Галерея самолёта',
  allCabins: 'Все кабины',
  seatsIn: 'Места в',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'у окна',
  seatPositionAisle: 'у прохода',
  seatPositionMiddle: 'среднее',
  seatExtraLegroom: 'увеличенное пространство для ног',
  seatExitRow: 'аварийный выход',
  seatAvailable: 'доступно',
  seatUnavailable: 'недоступно',
  seatSelected: 'выбрано',
  seatSelectedFor: 'выбрано для',
  seatRestrictedFor: 'недоступно для',
  close: 'Закрыть',
  moveToSeat: 'Перейти к месту',
  gridLabel: 'Схема мест',
  allSeats: 'Все места',
  row: 'Ряд',
  seat: 'Место',
  cabin: 'Кабина',
  position: 'Расположение',
  features: 'Удобства',
  price: 'Цена',
  status: 'Статус',
  action: 'Действие',
};

export const LOCALIZATION_CN: Record<string, string> = {
  deck: '甲板',
  cancel: '取消',
  select: '選擇',
  unselect: '取消選擇',
  available: '可用',
  unavailable: '不可用',
  selected: '已选择',
  preferred: '优选',
  extra: '额外',
  disabled: '已禁用',
  pitch: '座位间距',
  width: '座位宽度',
  recline: '倾斜度',
  wifi: 'Wi-Fi',
  wifi_enabled: '有Wi-Fi',
  wifiEnabled: '有Wi-Fi',
  power: '电源插座',
  powerPlug: '电源插座',
  usbPlug: 'USB端口',
  usbPowerPlug: 'USB端口及电源插座',
  audioVideo: '自选式机上视听娱乐系统',
  audio_video_ondemand: '自选式机上视听娱乐系统',
  entertainment: '自选式机上视听娱乐系统',
  bluetooth: '配对您的耳麦',
  restricted_legroom: '腿部空间较小',
  restrictedLegroom: '腿部空间较小',
  extra_legroom: '腿部空间较大',
  extraLegroom: '腿部空间较大',
  noData: '无座位图可用',
  loading: '正在加载座位图...',
  first: '头等舱',
  business: '商务舱',
  premium: '特等舱',
  economy: '经济舱',
  F: '头等舱',
  B: '商务舱',
  P: '特等舱',
  E: '经济舱',
  babyBassinet: '有婴儿摇篮',
  bassinet: '有婴儿摇篮',
  doNotRecline: '后仰角度小，可能背疼',
  limitedRecline: '后仰角度小，可能背疼',
  nearGalley: '靠近厨房',
  nearLavatory: '靠近洗手间',
  nearStairs: '楼梯，来往人流较多',
  noFloorStorage: '前面无座位',
  noOverheadStorage: '行李柜较小',
  limitedOverheadStorage: '行李柜较小',
  getColdByExit: '靠近紧急出口，较冷',
  misalignedWindow: '部分窗户或无窗户',
  noWindow: '部分窗户或无窗户',
  wingInWindow: '机翼在窗外视线内',
  trayTableInArmrest: '托盘桌在扶手里',
  storageBoxBetweenWall: '牆間儲物箱',
  standardSeat: '标准座位',
  reservedCrewSeat: '已预订',
  prereclinedSeat: '预倾斜座椅',
  personalTvInArmrest: '扶手里的个人电视',
  invertedSeat: '后向式座椅',
  narrower: '较窄的座椅',
  exitRow: '出口行',
  ADT: '成人',
  CHD: '儿童',
  INF: '婴儿',
  seatRestrictions: '该座位仅用于',
  passenger: '乘客',
  passengers: '乘客',
  legend: '图例',
  free: '免费',
  gallery: '飞机图库',
  allCabins: '所有客舱',
  seatsIn: '座位在',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: '靠窗',
  seatPositionAisle: '靠过道',
  seatPositionMiddle: '中间',
  seatExtraLegroom: '宽敞腿部空间',
  seatExitRow: '紧急出口排',
  seatAvailable: '可选',
  seatUnavailable: '不可用',
  seatSelected: '已选',
  seatSelectedFor: '已选给',
  seatRestrictedFor: '不可用于',
  close: '关闭',
  moveToSeat: '前往座位',
  gridLabel: '座位图',
  allSeats: '所有座位',
  row: '排',
  seat: '座位',
  cabin: '客舱',
  position: '位置',
  features: '功能',
  price: '价格',
  status: '状态',
  action: '操作',
};

export const LOCALIZATION_DE: Record<string, string> = {
  deck: 'Deck',
  cancel: 'Abbrechen',
  select: 'Auswählen',
  unselect: 'Auswahl aufheben',
  available: 'Verfügbar',
  unavailable: 'Nicht verfügbar',
  selected: 'Ausgewählt',
  preferred: 'Bevorzugt',
  extra: 'Extra',
  disabled: 'Deaktiviert',
  pitch: 'Abstand',
  width: 'Breite',
  recline: 'Rücklehne',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi aktiviert',
  wifiEnabled: 'Wi-Fi aktiviert',
  power: 'Stromanschluss (%s)',
  powerPlug: 'Stromanschluss (%s)',
  usbPlug: 'USB Anschluss',
  usbPowerPlug: 'USB und Stromanschluss (%s)',
  audioVideo: 'Audio und Video auf Anfrage (%s)',
  audio_video_ondemand: 'Audio und Video auf Anfrage (%s)',
  entertainment: 'Audio und Video auf Anfrage',
  bluetooth: 'Koppeln Sie Ihr Headset',
  restricted_legroom: 'Eingeschränkte Beinfreiheit',
  restrictedLegroom: 'Eingeschränkte Beinfreiheit',
  extra_legroom: 'Zusätzliche Beinfreiheit',
  extraLegroom: 'Mehr Beinfreiheit',
  noData: 'Keine Sitzplatzkarte verfügbar',
  loading: 'Sitzplatzkarte wird geladen...',
  first: 'First Class',
  business: 'Business Class',
  premium: 'Premium Economy',
  economy: 'Economy',
  F: 'Erste Klasse',
  B: 'Business-Klasse',
  P: 'Premiumklasse',
  E: 'Economy-Klasse',
  babyBassinet: 'Babywiege vorhanden',
  bassinet: 'Babywiege vorhanden',
  doNotRecline: 'Eingeschränkte Liegeposition, Rückenschmerzen möglich',
  limitedRecline: 'Eingeschränkte Liegeposition, Rückenschmerzen möglich',
  nearGalley: 'In der Nähe von der Bordküche',
  nearLavatory: 'In der Nähe von Toiletten',
  nearStairs: 'Treppen, stark frequentierte Verkehrsfläche',
  noFloorStorage: 'Kein Stauraum unter dem Sitz',
  noOverheadStorage: 'Kein Stauraum in den oberen Gepäckfächern',
  limitedOverheadStorage: 'Stauraum in den oberen Gepäckfächern begrenzt',
  getColdByExit: 'Nah zum Ausgang, Zugluft, kalt',
  misalignedWindow: 'Eingeschränkter oder fehlender Fensterblick',
  noWindow: 'Kein Fensterblick',
  wingInWindow: 'Fensterblick auf Flügel',
  trayTableInArmrest: 'Tablett-Tisch in der Armlehne',
  storageBoxBetweenWall: 'Aufbewahrungsbox zwischen Wand',
  standardSeat: 'Standardsitz',
  reservedCrewSeat: 'Reserviert',
  prereclinedSeat: 'Vorgelehnter sitz',
  personalTvInArmrest: 'Personal TV in der Armlehne',
  invertedSeat: 'Nach hinten gerichteter Sitz',
  narrower: 'Engerer Sitz',
  exitRow: 'Ausstiegsreihe',
  ADT: 'Erwachsene',
  CHD: 'Kinder',
  INF: 'Kleinkinder',
  seatRestrictions: 'Der Sitz ist nur für',
  passenger: 'Passagier',
  passengers: 'Passagiere',
  legend: 'Legende',
  free: 'kostenlos',
  gallery: 'Flugzeuggalerie',
  allCabins: 'Alle Kabinen',
  seatsIn: 'Sitze in',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'Fenster',
  seatPositionAisle: 'Gang',
  seatPositionMiddle: 'Mitte',
  seatExtraLegroom: 'mehr Beinfreiheit',
  seatExitRow: 'Notausgangsreihe',
  seatAvailable: 'verfügbar',
  seatUnavailable: 'nicht verfügbar',
  seatSelected: 'ausgewählt',
  seatSelectedFor: 'ausgewählt für',
  seatRestrictedFor: 'nicht verfügbar für',
  close: 'Schließen',
  moveToSeat: 'Zum Sitz',
  gridLabel: 'Sitzplan',
  allSeats: 'Alle Sitze',
  row: 'Reihe',
  seat: 'Sitz',
  cabin: 'Kabine',
  position: 'Position',
  features: 'Ausstattung',
  price: 'Preis',
  status: 'Status',
  action: 'Aktion',
};

export const LOCALIZATION_FR: Record<string, string> = {
  deck: 'Pont',
  cancel: 'Annuler',
  select: 'Sélectionner',
  unselect: 'Désélectionner',
  available: 'Disponible',
  unavailable: 'Indisponible',
  selected: 'Sélectionné',
  preferred: 'Préféré',
  extra: 'Extra',
  disabled: 'Désactivé',
  pitch: 'Inclinaison',
  width: 'Largeur',
  recline: 'Incliner',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi activé',
  wifiEnabled: 'Wi-Fi activé',
  power: 'Prise électrique',
  powerPlug: 'Prise électrique',
  usbPlug: 'Prise USB',
  usbPowerPlug: 'Prise USB et électrique',
  audioVideo: 'Audio et vidéo à la demande',
  audio_video_ondemand: 'Audio et vidéo à la demande',
  entertainment: 'Audio et vidéo à la demande',
  bluetooth: 'Appairer votre casque',
  restricted_legroom: 'Espace pour les jambes limité',
  restrictedLegroom: 'Espace pour les jambes limité',
  extra_legroom: 'Espace supplémentaire pour les jambes',
  extraLegroom: 'Espace supplémentaire pour les jambes',
  noData: 'Aucun plan de cabine disponible',
  loading: 'Chargement du plan de cabine...',
  first: 'Première classe',
  business: 'Classe affaires',
  premium: 'Classe Premium',
  economy: 'Classe économique',
  F: 'Première classe',
  B: 'Classe affaires',
  P: 'Classe Premium',
  E: 'Classe économique',
  babyBassinet: 'Lit bébé disponible',
  bassinet: 'Lit bébé disponible',
  doNotRecline: 'Inclinaison limitée, possibles maux de dos possibles',
  limitedRecline: 'Inclinaison limitée, possibles maux de dos possibles',
  nearGalley: 'Près de la cuisine',
  nearLavatory: 'Près des toilettes',
  nearStairs: 'Escaliers, zone de fort passage',
  noFloorStorage: 'Pas de rangement sous le siège',
  noOverheadStorage: 'Espace de rangement en hauteur limité',
  limitedOverheadStorage: 'Espace de rangement en hauteur limité',
  getColdByExit: 'Près de la sortie, courants d’air et froid',
  misalignedWindow: 'Vue partielle ou manquante par la fenêtre',
  noWindow: 'Vue partielle ou manquante par la fenêtre',
  wingInWindow: 'Vue de l’aile par le hublot',
  trayTableInArmrest: 'Tablette dans l’accoudoir',
  storageBoxBetweenWall: 'Compartiment de rangement entre les cloisons',
  standardSeat: 'Siège standard',
  reservedCrewSeat: 'Réservé',
  prereclinedSeat: 'Siège pré-incliné',
  personalTvInArmrest: 'TV personnelle dans l’accoudoir',
  invertedSeat: 'Siège orienté vers l’arrière',
  narrower: 'Siège plus étroit',
  exitRow: 'Rangée côté sortie',
  ADT: 'adultes',
  CHD: 'enfants',
  INF: 'nourrissons',
  seatRestrictions: 'Le siège est uniquement pour',
  passenger: 'Passager',
  passengers: 'Passagers',
  legend: 'Légende',
  free: 'gratuit',
  gallery: 'Galerie de l’avion',
  allCabins: 'Toutes les cabines',
  seatsIn: 'Places en',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'hublot',
  seatPositionAisle: 'couloir',
  seatPositionMiddle: 'milieu',
  seatExtraLegroom: 'espace jambes accru',
  seatExitRow: 'rangée de sortie',
  seatAvailable: 'disponible',
  seatUnavailable: 'indisponible',
  seatSelected: 'sélectionné',
  seatSelectedFor: 'sélectionné pour',
  seatRestrictedFor: 'indisponible pour',
  close: 'Fermer',
  moveToSeat: 'Aller au siège',
  gridLabel: 'Plan des sièges',
  allSeats: 'Tous les sièges',
  row: 'Rangée',
  seat: 'Siège',
  cabin: 'Cabine',
  position: 'Position',
  features: 'Caractéristiques',
  price: 'Prix',
  status: 'Statut',
  action: 'Action',
};

// Canadian French. Shared keys mirror jets-seatmap-react-lib-pub's
// LOCALE_FR_CA (i18n.languages.js, PR #171); the Angular-only keys
// (status labels, legend/gallery/loading, snake_case legroom synonyms)
// reuse the closest LOCALIZATION_FR string so the dict matches the
// shape of every other Angular locale.
export const LOCALIZATION_FR_CA: Record<string, string> = {
  deck: 'Pont',
  cancel: 'Annuler',
  select: 'Sélectionner',
  unselect: 'Désélectionner',
  available: 'Disponible',
  unavailable: 'Indisponible',
  selected: 'Sélectionné',
  preferred: 'Préféré',
  extra: 'Extra',
  disabled: 'Désactivé',
  pitch: 'Inclinaison',
  width: 'Largeur',
  recline: 'Inclinaison',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi disponible',
  wifiEnabled: 'Wi-Fi disponible',
  power: 'Prise de courant',
  powerPlug: 'Prise de courant',
  usbPlug: 'Prise USB',
  usbPowerPlug: 'Prises USB et de courant',
  audioVideo: 'Divertissement sur demande',
  audio_video_ondemand: 'Divertissement sur demande',
  entertainment: 'Divertissement sur demande',
  bluetooth: 'Jumeler votre casque',
  restricted_legroom: 'Espace restreint pour les jambes',
  restrictedLegroom: 'Espace restreint pour les jambes',
  extra_legroom: 'Espace supplémentaire pour les jambes',
  extraLegroom: 'Espace supplémentaire pour les jambes',
  noData: 'Aucun plan de cabine disponible',
  loading: 'Chargement du plan de cabine...',
  first: 'Première classe',
  business: 'Classe affaires',
  premium: 'Classe Premium',
  economy: 'Classe économique',
  F: 'Première classe',
  B: 'Classe affaires',
  P: 'Classe Premium',
  E: 'Classe économique',
  babyBassinet: 'Moïse disponible',
  bassinet: 'Moïse disponible',
  doNotRecline: 'Inclinaison restreinte (confort dorsal limité)',
  limitedRecline: 'Inclinaison restreinte (confort dorsal limité)',
  nearGalley: 'Près de l’office',
  nearLavatory: 'Près des toilettes',
  nearStairs: 'Près de l’escalier (zone passante)',
  noFloorStorage: 'Aucun rangement sous le siège',
  noOverheadStorage: 'Espace de rangement supérieur limité',
  limitedOverheadStorage: 'Espace de rangement supérieur limité',
  getColdByExit: 'Près de la sortie (courants d’air possibles)',
  misalignedWindow: 'Vue partielle ou aucune vue par la fenêtre',
  noWindow: 'Sans fenêtre',
  wingInWindow: 'Vue de l’aile par la fenêtre',
  trayTableInArmrest: 'Tablette dans l’accoudoir',
  storageBoxBetweenWall: 'Compartiment de rangement mural',
  standardSeat: 'Siège standard',
  reservedCrewSeat: 'Réservé',
  prereclinedSeat: 'Siège pré-incliné',
  personalTvInArmrest: 'Écran personnel dans l’accoudoir',
  invertedSeat: 'Siège orienté vers l’arrière',
  narrower: 'Siège plus étroit',
  exitRow: 'Rangée de sortie de secours',
  ADT: 'adultes',
  CHD: 'enfants',
  INF: 'bébés',
  seatRestrictions: 'Ce siège est réservé aux',
  passenger: 'Passager',
  passengers: 'Passagers',
  legend: 'Légende',
  free: 'gratuit',
  gallery: 'Galerie de l’avion',
  allCabins: 'Toutes les cabines',
  seatsIn: 'Places en',

  // WCAG a11y keys (commit 3) — Canadian French mirrors FR
  seatPositionWindow: 'hublot',
  seatPositionAisle: 'couloir',
  seatPositionMiddle: 'milieu',
  seatExtraLegroom: 'espace jambes accru',
  seatExitRow: 'rangée de sortie',
  seatAvailable: 'disponible',
  seatUnavailable: 'indisponible',
  seatSelected: 'sélectionné',
  seatSelectedFor: 'sélectionné pour',
  seatRestrictedFor: 'indisponible pour',
  close: 'Fermer',
  moveToSeat: 'Aller au siège',
  gridLabel: 'Plan des sièges',
  allSeats: 'Tous les sièges',
  row: 'Rangée',
  seat: 'Siège',
  cabin: 'Cabine',
  position: 'Position',
  features: 'Caractéristiques',
  price: 'Prix',
  status: 'Statut',
  action: 'Action',
};

export const LOCALIZATION_ES: Record<string, string> = {
  deck: 'Suelo',
  cancel: 'Cancelar',
  select: 'Seleccionar',
  unselect: 'Deseleccionar',
  available: 'Disponible',
  unavailable: 'No disponible',
  selected: 'Seleccionado',
  preferred: 'Preferido',
  extra: 'Extra',
  disabled: 'Desactivado',
  pitch: 'Separación',
  width: 'Anchura',
  recline: 'Reclinación',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi habilitado',
  wifiEnabled: 'Wi-Fi habilitado',
  power: 'Enchufe (%s)',
  powerPlug: 'Enchufe (%s)',
  usbPlug: 'Enchufe USB',
  usbPowerPlug: 'USB y enchufe (%s)',
  audioVideo: 'Audio y vídeo bajo demanda (%s)',
  audio_video_ondemand: 'Audio y vídeo bajo demanda (%s)',
  entertainment: 'Audio y vídeo bajo demanda',
  bluetooth: 'Emparejar los auriculares',
  restricted_legroom: 'Espacio para las piernas restringido',
  restrictedLegroom: 'Espacio para las piernas restringido',
  extra_legroom: 'Espacio extra para las piernas',
  extraLegroom: 'Espacio extra para las piernas',
  noData: 'No hay plano de asientos disponible',
  loading: 'Cargando plano de asientos...',
  first: 'Primera clase',
  business: 'Clase Business',
  premium: 'Clase superior',
  economy: 'Clase turista',
  F: 'Primera clase',
  B: 'Clase Business',
  P: 'Clase superior',
  E: 'Clase turista',
  babyBassinet: 'Moisés para bebé disponible',
  bassinet: 'Moisés para bebé disponible',
  doNotRecline: 'Posibles dolores de espalda por reclinación limitada',
  limitedRecline: 'Posibles dolores de espalda por reclinación limitada',
  nearGalley: 'Cerca de las cocinas',
  nearLavatory: 'Cerca de los aseos',
  nearStairs: 'Zona de escaleras de mucho tráfico',
  noFloorStorage: 'Sin almacenamiento debajo del asiento',
  noOverheadStorage: 'Espacio de almacenamiento limitado',
  limitedOverheadStorage: 'Espacio de almacenamiento limitado',
  getColdByExit: 'Cerca de la salida, corrientes de aire y frío',
  misalignedWindow: 'Vista parcial o nula por la ventana',
  noWindow: 'Vista parcial o nula por la ventana',
  wingInWindow: 'Vista del ala desde la ventana',
  trayTableInArmrest: 'Bandeja plegable en el reposabrazos',
  storageBoxBetweenWall: 'Caja de almacenamiento entre la pared',
  standardSeat: 'Asiento estándar',
  reservedCrewSeat: 'Reservado',
  prereclinedSeat: 'Asiento prerreclinado',
  personalTvInArmrest: 'TV personal en el reposabrazos',
  invertedSeat: 'Asiento orientado hacia atrás',
  narrower: 'Asiento más estrecho',
  exitRow: 'Fila de salida',
  ADT: 'adultos',
  CHD: 'niños',
  INF: 'bebés',
  seatRestrictions: 'Asiento solo para',
  passenger: 'Pasajeros',
  passengers: 'Pasajeros',
  legend: 'Leyenda',
  free: 'gratis',
  gallery: 'Galería del avión',
  allCabins: 'Todas las cabinas',
  seatsIn: 'Asientos en',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'ventana',
  seatPositionAisle: 'pasillo',
  seatPositionMiddle: 'central',
  seatExtraLegroom: 'mayor espacio para piernas',
  seatExitRow: 'fila de salida',
  seatAvailable: 'disponible',
  seatUnavailable: 'no disponible',
  seatSelected: 'seleccionado',
  seatSelectedFor: 'seleccionado para',
  seatRestrictedFor: 'no disponible para',
  close: 'Cerrar',
  moveToSeat: 'Ir al asiento',
  gridLabel: 'Mapa de asientos',
  allSeats: 'Todos los asientos',
  row: 'Fila',
  seat: 'Asiento',
  cabin: 'Cabina',
  position: 'Posición',
  features: 'Características',
  price: 'Precio',
  status: 'Estado',
  action: 'Acción',
};

export const LOCALIZATION_IT: Record<string, string> = {
  deck: 'Ponte',
  cancel: 'Annulla',
  select: 'Seleziona',
  unselect: 'Deseleziona',
  available: 'Disponibile',
  unavailable: 'Non disponibile',
  selected: 'Selezionato',
  preferred: 'Preferito',
  extra: 'Extra',
  disabled: 'Disabilitato',
  pitch: 'Inclinazione',
  width: 'Larghezza',
  recline: 'Reclinare',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Abilitato Wi-Fi',
  wifiEnabled: 'Abilitato Wi-Fi',
  power: 'Presa di corrente',
  powerPlug: 'Presa di corrente',
  usbPlug: 'Presa USB',
  usbPowerPlug: 'USB e presa di corrente',
  audioVideo: 'Audio e video su richiesta',
  audio_video_ondemand: 'Audio e video su richiesta',
  entertainment: 'Audio e video su richiesta',
  bluetooth: 'Accoppiamento delle cuffie',
  restricted_legroom: 'Spazio per le gambe limitato',
  restrictedLegroom: 'Spazio per le gambe limitato',
  extra_legroom: 'Spazio extra per le gambe',
  extraLegroom: 'Spazio extra per le gambe',
  noData: 'Nessuna mappa posti disponibile',
  loading: 'Caricamento della mappa posti...',
  first: 'Prima classe',
  business: 'Classe business',
  premium: 'Classe Premium',
  economy: 'Classe economica',
  F: 'Prima classe',
  B: 'Classe business',
  P: 'Classe Premium',
  E: 'Classe economica',
  babyBassinet: 'Culla per neonati disponibile',
  bassinet: 'Culla per neonati disponibile',
  doNotRecline: 'Reclinazione limitata, possibili mal di schiena',
  limitedRecline: 'Reclinazione limitata, possibili mal di schiena',
  nearGalley: 'Vicino alla cucina',
  nearLavatory: 'Vicino ai servizi igienici',
  nearStairs: 'Scale, zona di forte passaggio',
  noFloorStorage: 'Nessuno spazio di stivaggio sotto il sedile',
  noOverheadStorage: 'Spazio di stivaggio limitato',
  limitedOverheadStorage: 'Spazio di stivaggio limitato',
  getColdByExit: 'Vicino all’uscita, correnti d’aria e freddo',
  misalignedWindow: 'Vista finestrino parziale o assente',
  noWindow: 'Vista finestrino parziale o assente',
  wingInWindow: 'Vista ala da finestrino',
  trayTableInArmrest: 'Tavolino nel bracciolo',
  storageBoxBetweenWall: 'Scatola di stivaggio tra le pareti',
  standardSeat: 'Sedile standard',
  reservedCrewSeat: 'Riservato',
  prereclinedSeat: 'Sedile pre-reclinato',
  personalTvInArmrest: 'TV personale nel bracciolo',
  invertedSeat: 'Sedile rivolto all’indietro',
  narrower: 'Sedile più stretto',
  exitRow: 'Fila di uscita',
  ADT: 'adulti',
  CHD: 'bambini',
  INF: 'neonati',
  seatRestrictions: 'Il sedile è solo per',
  passenger: 'Passeggero',
  passengers: 'Passeggeri',
  legend: 'Legenda',
  free: 'gratis',
  gallery: 'Galleria dell’aereo',
  allCabins: 'Tutte le cabine',
  seatsIn: 'Posti in',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'finestrino',
  seatPositionAisle: 'corridoio',
  seatPositionMiddle: 'centrale',
  seatExtraLegroom: 'maggiore spazio per le gambe',
  seatExitRow: 'fila di uscita',
  seatAvailable: 'disponibile',
  seatUnavailable: 'non disponibile',
  seatSelected: 'selezionato',
  seatSelectedFor: 'selezionato per',
  seatRestrictedFor: 'non disponibile per',
  close: 'Chiudi',
  moveToSeat: 'Vai al posto',
  gridLabel: 'Mappa dei posti',
  allSeats: 'Tutti i posti',
  row: 'Fila',
  seat: 'Posto',
  cabin: 'Cabina',
  position: 'Posizione',
  features: 'Caratteristiche',
  price: 'Prezzo',
  status: 'Stato',
  action: 'Azione',
};

export const LOCALIZATION_PT: Record<string, string> = {
  deck: 'Convés',
  cancel: 'Cancelar',
  select: 'Selecionar',
  unselect: 'Desmarcar',
  available: 'Disponível',
  unavailable: 'Indisponível',
  selected: 'Selecionado',
  preferred: 'Preferido',
  extra: 'Extra',
  disabled: 'Desativado',
  pitch: 'Inclinação',
  width: 'Largura',
  recline: 'Reclinar',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi habilitado',
  wifiEnabled: 'Wi-Fi habilitado',
  power: 'Tomada',
  powerPlug: 'Tomada',
  usbPlug: 'Tomada USB',
  usbPowerPlug: 'USB e tomada',
  audioVideo: 'Áudio e vídeo sob demanda',
  audio_video_ondemand: 'Áudio e vídeo sob demanda',
  entertainment: 'Áudio e vídeo sob demanda',
  bluetooth: 'Emparelhar o auricular',
  restricted_legroom: 'Espaço para as pernas restrito',
  restrictedLegroom: 'Espaço para as pernas restrito',
  extra_legroom: 'Espaço extra para as pernas',
  extraLegroom: 'Espaço extra para as pernas',
  noData: 'Sem mapa de assentos disponível',
  loading: 'Carregando mapa de assentos...',
  first: 'Primeira classe',
  business: 'Classe executiva',
  premium: 'Classe Premium',
  economy: 'Classe económica',
  F: 'Primeira classe',
  B: 'Classe executiva',
  P: 'Classe Premium',
  E: 'Classe económica',
  babyBassinet: 'Berço para bebê disponível',
  bassinet: 'Berço para bebê disponível',
  doNotRecline: 'Reclinação restrita, possíveis dores nas costas',
  limitedRecline: 'Reclinação restrita, possíveis dores nas costas',
  nearGalley: 'Perto da cozinha',
  nearLavatory: 'Perto do lavatório',
  nearStairs: 'Escadas, área de tráfego intenso',
  noFloorStorage: 'Sem armazenamento sob o assento',
  noOverheadStorage: 'Armazenamento superior limitado',
  limitedOverheadStorage: 'Armazenamento superior limitado',
  getColdByExit: 'Perto da saída, correntes de ar e frio',
  misalignedWindow: 'Vista parcial ou ausente da janela',
  noWindow: 'Vista parcial ou ausente da janela',
  wingInWindow: 'Vista da asa da janela',
  trayTableInArmrest: 'Mesa de bandeja no apoio de braço',
  storageBoxBetweenWall: 'Caixa de armazenamento entre a parede',
  standardSeat: 'Assento padrão',
  reservedCrewSeat: 'Reservado',
  prereclinedSeat: 'Assento pré-reclinado',
  personalTvInArmrest: 'TV pessoal no apoio de braço',
  invertedSeat: 'Assento virado para trás',
  narrower: 'Assento mais estreito',
  exitRow: 'Fila de saída',
  ADT: 'adultos',
  CHD: 'crianças',
  INF: 'bebês',
  seatRestrictions: 'O assento é apenas para',
  passenger: 'Passageiro',
  passengers: 'Passageiros',
  legend: 'Legenda',
  free: 'grátis',
  gallery: 'Galeria da aeronave',
  allCabins: 'Todas as cabines',
  seatsIn: 'Assentos em',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'janela',
  seatPositionAisle: 'corredor',
  seatPositionMiddle: 'meio',
  seatExtraLegroom: 'mais espaço para as pernas',
  seatExitRow: 'fila de saída',
  seatAvailable: 'disponível',
  seatUnavailable: 'indisponível',
  seatSelected: 'selecionado',
  seatSelectedFor: 'selecionado para',
  seatRestrictedFor: 'indisponível para',
  close: 'Fechar',
  moveToSeat: 'Ir para o lugar',
  gridLabel: 'Mapa de lugares',
  allSeats: 'Todos os lugares',
  row: 'Fila',
  seat: 'Lugar',
  cabin: 'Cabine',
  position: 'Posição',
  features: 'Características',
  price: 'Preço',
  status: 'Estado',
  action: 'Ação',
};

export const LOCALIZATION_PT_BR: Record<string, string> = {
  deck: 'Andar',
  cancel: 'Cancelar',
  select: 'Selecionar',
  unselect: 'Desmarcar',
  available: 'Disponível',
  unavailable: 'Indisponível',
  selected: 'Selecionado',
  preferred: 'Preferido',
  extra: 'Extra',
  disabled: 'Desativado',
  pitch: 'Espaço',
  width: 'Largura',
  recline: 'Inclinação',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi disponível',
  wifiEnabled: 'Wi-Fi disponível',
  power: 'Tomada de energia',
  powerPlug: 'Tomada de energia',
  usbPlug: 'Entrada USB',
  usbPowerPlug: 'Entrada USB e tomada de energia',
  audioVideo: 'Áudio e vídeo sob demanda',
  audio_video_ondemand: 'Áudio e vídeo sob demanda',
  entertainment: 'Áudio e vídeo sob demanda',
  bluetooth: 'Conecte seu fone de ouvido',
  restricted_legroom: 'Espaço para as pernas limitado',
  restrictedLegroom: 'Espaço para as pernas limitado',
  extra_legroom: 'Mais espaço para as pernas',
  extraLegroom: 'Mais espaço para as pernas',
  noData: 'Sem mapa de assentos disponível',
  loading: 'Carregando mapa de assentos...',
  first: 'Primeira classe',
  business: 'Classe executiva',
  premium: 'Classe premium',
  economy: 'Classe econômica',
  F: 'Primeira classe',
  B: 'Classe executiva',
  P: 'Classe premium',
  E: 'Classe econômica',
  babyBassinet: 'Berço para bebê disponível',
  bassinet: 'Berço para bebê disponível',
  doNotRecline: 'Inclinação restrita, possível desconforto nas costas',
  limitedRecline: 'Inclinação restrita, possível desconforto nas costas',
  nearGalley: 'Próximo às cozinhas',
  nearLavatory: 'Próximo aos banheiros',
  nearStairs: 'Escadas, área de tráfego intenso',
  noFloorStorage: 'Sem espaço sob o assento',
  noOverheadStorage: 'Espaço de armazenamento limitado',
  limitedOverheadStorage: 'Espaço de armazenamento limitado',
  getColdByExit: 'Próximo à saída, correntes de ar e frio',
  misalignedWindow: 'Vista parcial ou sem janela',
  noWindow: 'Vista parcial ou sem janela',
  wingInWindow: 'Vista da asa pela janela',
  trayTableInArmrest: 'Mesa dobrável no apoio de braço',
  storageBoxBetweenWall: 'Caixa de armazenamento entre a parede',
  standardSeat: 'Assento padrão',
  reservedCrewSeat: 'Reservado',
  prereclinedSeat: 'Assento pré-reclinado',
  personalTvInArmrest: 'TV pessoal no apoio de braço',
  invertedSeat: 'Assento voltado para trás',
  narrower: 'Assento mais estreito',
  exitRow: 'Saída de emergência',
  ADT: 'adultos',
  CHD: 'crianças',
  INF: 'bebês',
  seatRestrictions: 'O assento é apenas para',
  passenger: 'Passageiro',
  passengers: 'Passageiros',
  legend: 'Legenda',
  free: 'grátis',
  gallery: 'Galeria do avião',
  allCabins: 'Todas as cabines',
  seatsIn: 'Assentos em',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'janela',
  seatPositionAisle: 'corredor',
  seatPositionMiddle: 'meio',
  seatExtraLegroom: 'mais espaço para as pernas',
  seatExitRow: 'fileira de saída',
  seatAvailable: 'disponível',
  seatUnavailable: 'indisponível',
  seatSelected: 'selecionado',
  seatSelectedFor: 'selecionado para',
  seatRestrictedFor: 'indisponível para',
  close: 'Fechar',
  moveToSeat: 'Ir para o assento',
  gridLabel: 'Mapa de assentos',
  allSeats: 'Todos os assentos',
  row: 'Fileira',
  seat: 'Assento',
  cabin: 'Cabine',
  position: 'Posição',
  features: 'Recursos',
  price: 'Preço',
  status: 'Status',
  action: 'Ação',
};

export const LOCALIZATION_AR: Record<string, string> = {
  deck: 'سطح السفينة',
  cancel: 'إلغاء',
  select: 'اختر',
  unselect: 'إلغاء الاختيار',
  available: 'متاح',
  unavailable: 'غير متاح',
  selected: 'محدد',
  preferred: 'مفضل',
  extra: 'إضافي',
  disabled: 'معطل',
  pitch: 'ميل',
  width: 'عرض',
  recline: 'ميل',
  wifi: 'واي فاي',
  wifi_enabled: 'واي فاي مفعل',
  wifiEnabled: 'واي فاي مفعل',
  power: 'مأخذ كهرباء',
  powerPlug: 'مأخذ كهرباء',
  usbPlug: 'مأخذ USB',
  usbPowerPlug: 'USB ومأخذ كهرباء',
  audioVideo: 'الصوت والفيديو حسب الطلب',
  audio_video_ondemand: 'الصوت والفيديو حسب الطلب',
  entertainment: 'الصوت والفيديو حسب الطلب',
  bluetooth: 'إقران سماعة الرأس',
  restricted_legroom: 'مساحة محدودة للأرجل',
  restrictedLegroom: 'مساحة محدودة للأرجل',
  extra_legroom: 'مساحة إضافية للأرجل',
  extraLegroom: 'مساحة إضافية للأرجل',
  noData: 'لا توجد خريطة مقاعد متاحة',
  loading: 'جاري تحميل خريطة المقاعد...',
  first: 'الدرجة الأولى',
  business: 'درجة رجال الأعمال',
  premium: 'الفئة الممتازة',
  economy: 'الدرجة الاقتصادية',
  F: 'الدرجة الأولى',
  B: 'درجة رجال الأعمال',
  P: 'الفئة الممتازة',
  E: 'الدرجة الاقتصادية',
  babyBassinet: 'سرير الرضع متاح',
  bassinet: 'سرير الرضع متاح',
  doNotRecline: 'ميل محدود، ممكن ألم الظهر',
  limitedRecline: 'ميل محدود، ممكن ألم الظهر',
  nearGalley: 'بالقرب من المطبخ',
  nearLavatory: 'بالقرب من الحمام',
  nearStairs: 'الدرج، منطقة حركة كثيفة',
  noFloorStorage: 'لا يوجد مساحة تخزين تحت المقعد',
  noOverheadStorage: 'مساحة تخزين علوية محدودة',
  limitedOverheadStorage: 'مساحة تخزين علوية محدودة',
  getColdByExit: 'قرب الخروج، تيارات هوائية وبرودة',
  misalignedWindow: 'نافذة جزئية أو مفقودة',
  noWindow: 'نافذة جزئية أو مفقودة',
  wingInWindow: 'جناح من نافذة العرض',
  trayTableInArmrest: 'طاولة صينية في مسند الذراع',
  storageBoxBetweenWall: 'صندوق تخزين بين الجدران',
  standardSeat: 'مقعد قياسي',
  reservedCrewSeat: 'محجوز',
  prereclinedSeat: 'مقعد مائل مسبقًا',
  personalTvInArmrest: 'تلفزيون شخصي في مسند الذراع',
  invertedSeat: 'مقعد مواجه للخلف',
  narrower: 'مقعد أضيق',
  exitRow: 'صف الخروج',
  ADT: 'بالغين',
  CHD: 'أطفال',
  INF: 'رضع',
  seatRestrictions: 'المقعد مخصص فقط لـ',
  passenger: 'راكب',
  passengers: 'الركاب',
  legend: 'وسيلة الإيضاح',
  free: 'مجاني',
  gallery: 'معرض الطائرة',
  allCabins: 'جميع المقصورات',
  seatsIn: 'المقاعد في',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'بجوار النافذة',
  seatPositionAisle: 'بجوار الممر',
  seatPositionMiddle: 'في الوسط',
  seatExtraLegroom: 'مساحة إضافية للساقين',
  seatExitRow: 'صف مخرج الطوارئ',
  seatAvailable: 'متاح',
  seatUnavailable: 'غير متاح',
  seatSelected: 'مختار',
  seatSelectedFor: 'مختار لـ',
  seatRestrictedFor: 'غير متاح لـ',
  close: 'إغلاق',
  moveToSeat: 'الانتقال إلى المقعد',
  gridLabel: 'خريطة المقاعد',
  allSeats: 'جميع المقاعد',
  row: 'صف',
  seat: 'مقعد',
  cabin: 'المقصورة',
  position: 'الموضع',
  features: 'المزايا',
  price: 'السعر',
  status: 'الحالة',
  action: 'إجراء',
};

export const LOCALIZATION_JA: Record<string, string> = {
  deck: 'デッキ',
  cancel: 'キャンセル',
  select: '選択',
  unselect: '選択解除',
  available: '利用可能',
  unavailable: '利用不可',
  selected: '選択中',
  preferred: '優先',
  extra: '追加',
  disabled: '無効',
  pitch: 'ピッチ',
  width: '幅',
  recline: 'リクライニング',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi有効',
  wifiEnabled: 'Wi-Fi有効',
  power: '電源プラグ',
  powerPlug: '電源プラグ',
  usbPlug: 'USBプラグ',
  usbPowerPlug: 'USB＆電源プラグ',
  audioVideo: 'オンデマンドのオーディオ＆ビデオ',
  audio_video_ondemand: 'オンデマンドのオーディオ＆ビデオ',
  entertainment: 'オンデマンドのオーディオ＆ビデオ',
  bluetooth: 'ヘッドセットをペアリングする',
  restricted_legroom: '足元スペースが制限されています',
  restrictedLegroom: '足元スペースが制限されています',
  extra_legroom: '追加の足元スペース',
  extraLegroom: '追加の足元スペース',
  noData: '座席マップは利用できません',
  loading: '座席マップを読み込み中...',
  first: 'ファーストクラス',
  business: 'ビジネスクラス',
  premium: 'プレミアムクラス',
  economy: 'エコノミークラス',
  F: 'ファーストクラス',
  B: 'ビジネスクラス',
  P: 'プレミアムクラス',
  E: 'エコノミークラス',
  babyBassinet: 'ベビーベッド利用可能',
  bassinet: 'ベビーベッド利用可能',
  doNotRecline: 'リクライニング制限、背中の痛みの可能性',
  limitedRecline: 'リクライニング制限、背中の痛みの可能性',
  nearGalley: 'ギャレーの近く',
  nearLavatory: 'トイレの近く',
  nearStairs: '階段、交通量の多いエリア',
  noFloorStorage: '座席下の収納スペースなし',
  noOverheadStorage: 'オーバーヘッドの収納スペースが限られています',
  limitedOverheadStorage: 'オーバーヘッドの収納スペースが限られています',
  getColdByExit: '出口付近、通風と寒さ',
  misalignedWindow: '部分的または欠けている窓の眺め',
  noWindow: '部分的または欠けている窓の眺め',
  wingInWindow: '窓からの翼の眺め',
  trayTableInArmrest: 'アームレスト内のトレイテーブル',
  storageBoxBetweenWall: '壁の間の収納ボックス',
  standardSeat: '標準の座席',
  reservedCrewSeat: '予約済み',
  prereclinedSeat: '事前にリクライニングされた座席',
  personalTvInArmrest: 'アームレスト内の個人用テレビ',
  invertedSeat: '後ろ向きの座席',
  narrower: '狭いシート',
  exitRow: '出口列',
  ADT: '大人',
  CHD: '子供',
  INF: '乳児',
  seatRestrictions: '座席は次のみのため',
  passenger: '乗客',
  passengers: '乗客',
  legend: '凡例',
  free: '無料',
  gallery: '機内ギャラリー',
  allCabins: 'すべてのキャビン',
  seatsIn: '座席:',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: '窓側',
  seatPositionAisle: '通路側',
  seatPositionMiddle: '中央',
  seatExtraLegroom: '広い足元スペース',
  seatExitRow: '非常口列',
  seatAvailable: '空席',
  seatUnavailable: '利用不可',
  seatSelected: '選択済み',
  seatSelectedFor: '選択中の乗客',
  seatRestrictedFor: '利用不可の乗客タイプ',
  close: '閉じる',
  moveToSeat: '座席へ移動',
  gridLabel: '座席マップ',
  allSeats: 'すべての座席',
  row: '列',
  seat: '座席',
  cabin: 'キャビン',
  position: '位置',
  features: '設備',
  price: '価格',
  status: '状態',
  action: '操作',
};

export const LOCALIZATION_KO: Record<string, string> = {
  deck: '데크',
  cancel: '취소',
  select: '선택',
  unselect: '선택 취소',
  available: '사용 가능',
  unavailable: '사용 불가',
  selected: '선택됨',
  preferred: '선호',
  extra: '추가',
  disabled: '비활성화됨',
  pitch: '피치',
  width: '너비',
  recline: '기울기',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi 사용 가능',
  wifiEnabled: 'Wi-Fi 사용 가능',
  power: '전원 플러그',
  powerPlug: '전원 플러그',
  usbPlug: 'USB 플러그',
  usbPowerPlug: 'USB 및 전원 플러그',
  audioVideo: '요청 시 오디오 및 비디오',
  audio_video_ondemand: '요청 시 오디오 및 비디오',
  entertainment: '요청 시 오디오 및 비디오',
  bluetooth: '헤드셋 페어링',
  restricted_legroom: '제한된 다리 공간',
  restrictedLegroom: '제한된 다리 공간',
  extra_legroom: '추가 다리 공간',
  extraLegroom: '추가 다리 공간',
  noData: '좌석 지도를 사용할 수 없습니다',
  loading: '좌석 지도를 불러오는 중...',
  first: '퍼스트 클래스',
  business: '비즈니스 클래스',
  premium: '프리미엄 클래스',
  economy: '이코노미 클래스',
  F: '퍼스트 클래스',
  B: '비즈니스 클래스',
  P: '프리미엄 클래스',
  E: '이코노미 클래스',
  babyBassinet: '아기용 바스켓 사용 가능',
  bassinet: '아기용 바스켓 사용 가능',
  doNotRecline: '제한된 기울기, 허리 통증 가능성',
  limitedRecline: '제한된 기울기, 허리 통증 가능성',
  nearGalley: '갤리 근처',
  nearLavatory: '화장실 근처',
  nearStairs: '계단, 높은 트래픽 지역',
  noFloorStorage: '좌석 아래 보관 공간 없음',
  noOverheadStorage: '제한된 수납 공간',
  limitedOverheadStorage: '제한된 수납 공간',
  getColdByExit: '출구 근처, 통풍 및 추움',
  misalignedWindow: '부분적 또는 누락된 창 뷰',
  noWindow: '부분적 또는 누락된 창 뷰',
  wingInWindow: '창에서 날개 뷰',
  trayTableInArmrest: '팔걸이에 트레이 테이블',
  storageBoxBetweenWall: '벽 사이의 보관 상자',
  standardSeat: '표준 좌석',
  reservedCrewSeat: '예약됨',
  prereclinedSeat: '미리 기울어진 좌석',
  personalTvInArmrest: '팔걸이에 개인 TV',
  invertedSeat: '뒤집힌 좌석',
  narrower: '좁은 좌석',
  exitRow: '행 종료',
  ADT: '성인',
  CHD: '어린이',
  INF: '유아',
  seatRestrictions: '좌석은 오로지',
  passenger: '승객',
  passengers: '승객',
  legend: '범례',
  free: '무료',
  gallery: '항공기 갤러리',
  allCabins: '모든 캐빈',
  seatsIn: '좌석:',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: '창측',
  seatPositionAisle: '복도측',
  seatPositionMiddle: '가운데',
  seatExtraLegroom: '넓은 다리 공간',
  seatExitRow: '비상구 열',
  seatAvailable: '이용 가능',
  seatUnavailable: '이용 불가',
  seatSelected: '선택됨',
  seatSelectedFor: '선택된 승객',
  seatRestrictedFor: '이용 불가 승객',
  close: '닫기',
  moveToSeat: '좌석으로 이동',
  gridLabel: '좌석 맵',
  allSeats: '전체 좌석',
  row: '행',
  seat: '좌석',
  cabin: '캐빈',
  position: '위치',
  features: '기능',
  price: '가격',
  status: '상태',
  action: '작업',
};

export const LOCALIZATION_TR: Record<string, string> = {
  deck: 'Güverte',
  cancel: 'İptal',
  select: 'Seç',
  unselect: 'Seçimi kaldır',
  available: 'Müsait',
  unavailable: 'Müsait değil',
  selected: 'Seçildi',
  preferred: 'Tercih edilen',
  extra: 'Ekstra',
  disabled: 'Devre dışı',
  pitch: 'Eğim',
  width: 'Genişlik',
  recline: 'Yatma Derecesi',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi-Fi etkin',
  wifiEnabled: 'Wi-Fi etkin',
  power: 'Elektrik prizi',
  powerPlug: 'Elektrik prizi',
  usbPlug: 'USB prizi',
  usbPowerPlug: 'USB ve elektrik prizi',
  audioVideo: 'Talebe göre ses ve video',
  audio_video_ondemand: 'Talebe göre ses ve video',
  entertainment: 'Talebe göre ses ve video',
  bluetooth: 'Kulaklığınızı eşleştirin',
  restricted_legroom: 'Sınırlı bacak aralığı',
  restrictedLegroom: 'Sınırlı bacak aralığı',
  extra_legroom: 'Ekstra bacak aralığı',
  extraLegroom: 'Ekstra bacak aralığı',
  noData: 'Koltuk haritası mevcut değil',
  loading: 'Koltuk haritası yükleniyor...',
  first: 'Birinci sınıf',
  business: 'Business sınıfı',
  premium: 'Premium sınıf',
  economy: 'Ekonomi sınıfı',
  F: 'Birinci sınıf',
  B: 'Business sınıfı',
  P: 'Premium sınıf',
  E: 'Ekonomi sınıfı',
  babyBassinet: 'Bebek beşiği mevcut',
  bassinet: 'Bebek beşiği mevcut',
  doNotRecline: 'Sınırlı eğim, olası sırt ağrıları',
  limitedRecline: 'Sınırlı eğim, olası sırt ağrıları',
  nearGalley: 'Mutfak alanına yakın',
  nearLavatory: 'Tuvaletin yanında',
  nearStairs: 'Merdivenler, yoğun trafik alanı',
  noFloorStorage: 'Koltuk altı depolama alanı yok',
  noOverheadStorage: 'Sınırlı üst depolama alanı',
  limitedOverheadStorage: 'Sınırlı üst depolama alanı',
  getColdByExit: 'Çıkışa yakın, hava akımı ve soğuk',
  misalignedWindow: 'Kısmi veya eksik pencere görünümü',
  noWindow: 'Kısmi veya eksik pencere görünümü',
  wingInWindow: 'Pencereden kanat görünümü',
  trayTableInArmrest: 'Kol dayamada tepsi masası',
  storageBoxBetweenWall: 'Duvarlar arasında depolama kutusu',
  standardSeat: 'Standart koltuk',
  reservedCrewSeat: 'Rezerve',
  prereclinedSeat: 'Önceden eğimli koltuk',
  personalTvInArmrest: 'Kol dayama içinde kişisel TV',
  invertedSeat: 'Ters yönlü koltuk',
  narrower: 'Daha dar koltuk',
  exitRow: 'Çıkış sırası',
  ADT: 'yetişkinler',
  CHD: 'çocuklar',
  INF: 'bebekler',
  seatRestrictions: 'Koltuk sadece için',
  passenger: 'Yolcu',
  passengers: 'Yolcular',
  legend: 'Lejant',
  free: 'ücretsiz',
  gallery: 'Uçak galerisi',
  allCabins: 'Tüm kabinler',
  seatsIn: 'Koltuklar:',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'pencere',
  seatPositionAisle: 'koridor',
  seatPositionMiddle: 'orta',
  seatExtraLegroom: 'geniş bacak mesafesi',
  seatExitRow: 'çıkış sırası',
  seatAvailable: 'müsait',
  seatUnavailable: 'müsait değil',
  seatSelected: 'seçildi',
  seatSelectedFor: 'şunun için seçildi',
  seatRestrictedFor: 'şunun için müsait değil',
  close: 'Kapat',
  moveToSeat: 'Koltuğa git',
  gridLabel: 'Koltuk haritası',
  allSeats: 'Tüm koltuklar',
  row: 'Sıra',
  seat: 'Koltuk',
  cabin: 'Kabin',
  position: 'Konum',
  features: 'Özellikler',
  price: 'Fiyat',
  status: 'Durum',
  action: 'İşlem',
};

export const LOCALIZATION_NL: Record<string, string> = {
  deck: 'Dek',
  cancel: 'Annuleren',
  select: 'Selecteren',
  unselect: 'Deselecteren',
  available: 'Beschikbaar',
  unavailable: 'Niet beschikbaar',
  selected: 'Geselecteerd',
  preferred: 'Voorkeur',
  extra: 'Extra',
  disabled: 'Uitgeschakeld',
  pitch: 'Stoelafstand',
  width: 'Breedte',
  recline: 'Kanteling',
  wifi: 'Wi-Fi',
  wifi_enabled: 'Wi‑Fi beschikbaar',
  wifiEnabled: 'Wi‑Fi beschikbaar',
  power: 'Stroomaansluiting',
  powerPlug: 'Stroomaansluiting',
  usbPlug: 'USB‑aansluiting',
  usbPowerPlug: 'USB en stroomaansluiting',
  audioVideo: 'Audio en video op aanvraag',
  audio_video_ondemand: 'Audio en video op aanvraag',
  entertainment: 'Audio en video op aanvraag',
  bluetooth: 'Koppel uw headset',
  restricted_legroom: 'Beperkte beenruimte',
  restrictedLegroom: 'Beperkte beenruimte',
  extra_legroom: 'Extra beenruimte',
  extraLegroom: 'Extra beenruimte',
  noData: 'Geen stoelplan beschikbaar',
  loading: 'Stoelplan wordt geladen...',
  first: 'Eerste klasse',
  business: 'Businessklasse',
  premium: 'Premiumklasse',
  economy: 'Economyklasse',
  F: 'Eerste klasse',
  B: 'Businessklasse',
  P: 'Premiumklasse',
  E: 'Economyklasse',
  babyBassinet: 'Babywieg beschikbaar',
  bassinet: 'Babywieg beschikbaar',
  doNotRecline: 'Beperkte kanteling, kans op rugklachten',
  limitedRecline: 'Beperkte kanteling, kans op rugklachten',
  nearGalley: 'Dicht bij de kombuis',
  nearLavatory: 'Dicht bij de toiletten',
  nearStairs: 'Dicht bij de trap, druk gebied',
  noFloorStorage: 'Geen opbergruimte onder de stoel',
  noOverheadStorage: 'Beperkte opbergruimte',
  limitedOverheadStorage: 'Beperkte opbergruimte',
  getColdByExit: 'Dicht bij de uitgang, tocht en kou mogelijk',
  misalignedWindow: 'Gedeeltelijk of geen uitzicht door het raam',
  noWindow: 'Gedeeltelijk of geen uitzicht door het raam',
  wingInWindow: 'Vleugel zichtbaar vanuit het raam',
  trayTableInArmrest: 'Tafeltje in de armleuning',
  storageBoxBetweenWall: 'Opbergvak tussen wand',
  standardSeat: 'Standaardstoel',
  reservedCrewSeat: 'Gereserveerd',
  prereclinedSeat: 'Vooraf gekantelde stoel',
  personalTvInArmrest: 'Persoonlijke tv in de armleuning',
  invertedSeat: 'Achterwaarts gerichte stoel',
  narrower: 'Smallere stoel',
  exitRow: 'Nooduitgangrij',
  ADT: 'volwassenen',
  CHD: 'kinderen',
  INF: 'baby’s',
  seatRestrictions: 'De stoel is alleen voor',
  passenger: 'Passagier',
  passengers: 'Passagiers',
  legend: 'Legenda',
  free: 'gratis',
  gallery: 'Vliegtuiggalerij',
  allCabins: 'Alle cabines',
  seatsIn: 'Stoelen in',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'raam',
  seatPositionAisle: 'gangpad',
  seatPositionMiddle: 'midden',
  seatExtraLegroom: 'extra beenruimte',
  seatExitRow: 'nooduitgangrij',
  seatAvailable: 'beschikbaar',
  seatUnavailable: 'niet beschikbaar',
  seatSelected: 'geselecteerd',
  seatSelectedFor: 'geselecteerd voor',
  seatRestrictedFor: 'niet beschikbaar voor',
  close: 'Sluiten',
  moveToSeat: 'Naar stoel',
  gridLabel: 'Stoelplan',
  allSeats: 'Alle stoelen',
  row: 'Rij',
  seat: 'Stoel',
  cabin: 'Cabine',
  position: 'Positie',
  features: 'Voorzieningen',
  price: 'Prijs',
  status: 'Status',
  action: 'Actie',
};

export const LOCALIZATION_PL: Record<string, string> = {
  deck: 'Piętro',
  cancel: 'Anulować',
  select: 'Wybierać',
  unselect: 'Odznacz',
  available: 'Dostępne',
  unavailable: 'Niedostępne',
  selected: 'Wybrane',
  preferred: 'Preferowane',
  extra: 'Dodatkowe',
  disabled: 'Wyłączone',
  pitch: 'Pochylenie',
  width: 'Szerokość',
  recline: 'Rozkładany',
  wifi: 'Wi-Fi włączone',
  wifi_enabled: 'Wi-Fi włączone',
  wifiEnabled: 'Wi-Fi włączone',
  power: 'Wtyczka zasilania (%s)',
  powerPlug: 'Wtyczka zasilania (%s)',
  usbPlug: 'Wtyczka USB',
  usbPowerPlug: 'USB i wtyczka zasilania (%s)',
  audioVideo: 'Audio i wideo na żądanie (%s)',
  audio_video_ondemand: 'Audio i wideo na żądanie (%s)',
  entertainment: 'Audio i wideo na żądanie',
  bluetooth: 'Parowanie zestawu słuchawkowego',
  restricted_legroom: 'Ograniczona przestrzeń na nogi',
  restrictedLegroom: 'Ograniczona przestrzeń na nogi',
  extra_legroom: 'Dodatkowa przestrzeń na nogi',
  extraLegroom: 'Dodatkowa przestrzeń na nogi',
  noData: 'Mapa miejsc niedostępna',
  loading: 'Ładowanie mapy miejsc...',
  first: 'Pierwsza klasa',
  business: 'Klasa biznes',
  premium: 'Klasa premium',
  economy: 'Klasa ekonomiczna',
  F: 'Pierwsza klasa',
  B: 'Klasa biznes',
  P: 'Klasa premium',
  E: 'Klasa ekonomiczna',
  babyBassinet: 'Dostępne łóżeczko dziecięce',
  bassinet: 'Dostępne łóżeczko dziecięce',
  doNotRecline: 'Możliwe ograniczone odchylenie pleców',
  limitedRecline: 'Możliwe ograniczone odchylenie pleców',
  nearGalley: 'W pobliżu kuchni',
  nearLavatory: 'W pobliżu toalet',
  nearStairs: 'Obszar o dużym natężeniu ruchu na schodach',
  noFloorStorage: 'Brak miejsca na bagaż pod siedzeniem',
  noOverheadStorage: 'Ograniczona przestrzeń magazynowa',
  limitedOverheadStorage: 'Ograniczona przestrzeń magazynowa',
  getColdByExit: 'Blisko wyjść i w chłodzie',
  misalignedWindow: 'Częściowy lub brak widoku okna',
  noWindow: 'Częściowy lub brak widoku okna',
  wingInWindow: 'Skrzydło z okna',
  trayTableInArmrest: 'Taca w podłokietniku',
  storageBoxBetweenWall: 'Schowek między ścianą',
  standardSeat: 'Miejsce standardowe',
  reservedCrewSeat: 'Zarezerwowany',
  prereclinedSeat: 'Wstępnie rozkładane siedzenie',
  personalTvInArmrest: 'Telewizor osobisty w podłokietniku',
  invertedSeat: 'Fotel montowany tyłem do kierunku jazdy',
  narrower: 'Węższe siedzisko',
  exitRow: 'Rząd wyjściowy',
  ADT: 'dorośli',
  CHD: 'dzieci',
  INF: 'niemowlęta',
  seatRestrictions: 'Siedzenie jest przeznaczone tylko dla',
  passenger: 'Pasażer',
  passengers: 'Pasażerowie',
  legend: 'Legenda',
  free: 'darmowe',
  gallery: 'Galeria samolotu',
  allCabins: 'Wszystkie kabiny',
  seatsIn: 'Miejsca w',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'przy oknie',
  seatPositionAisle: 'przy przejściu',
  seatPositionMiddle: 'środkowe',
  seatExtraLegroom: 'większa przestrzeń na nogi',
  seatExitRow: 'rząd wyjścia awaryjnego',
  seatAvailable: 'dostępne',
  seatUnavailable: 'niedostępne',
  seatSelected: 'wybrane',
  seatSelectedFor: 'wybrane dla',
  seatRestrictedFor: 'niedostępne dla',
  close: 'Zamknij',
  moveToSeat: 'Przejdź do miejsca',
  gridLabel: 'Plan miejsc',
  allSeats: 'Wszystkie miejsca',
  row: 'Rząd',
  seat: 'Miejsce',
  cabin: 'Kabina',
  position: 'Położenie',
  features: 'Udogodnienia',
  price: 'Cena',
  status: 'Status',
  action: 'Akcja',
};

export const LOCALIZATION_CS: Record<string, string> = {
  deck: 'Paluba',
  cancel: 'Zrušit',
  select: 'Vybrat',
  unselect: 'Zrušit výběr',
  available: 'Dostupné',
  unavailable: 'Nedostupné',
  selected: 'Vybrané',
  preferred: 'Preferované',
  extra: 'Extra',
  disabled: 'Zakázáno',
  pitch: 'Naklonění',
  width: 'Šířka',
  recline: 'Naklonit',
  wifi: 'Wi-Fi povoleno',
  wifi_enabled: 'Wi-Fi povoleno',
  wifiEnabled: 'Wi-Fi povoleno',
  power: 'Elektrická zásuvka',
  powerPlug: 'Elektrická zásuvka',
  usbPlug: 'USB zásuvka',
  usbPowerPlug: 'USB a elektrická zásuvka',
  audioVideo: 'Audio a video na vyžádání',
  audio_video_ondemand: 'Audio a video na vyžádání',
  entertainment: 'Audio a video na vyžádání',
  bluetooth: 'Spárování náhlavní soupravy',
  restricted_legroom: 'Omezený prostor pro nohy',
  restrictedLegroom: 'Omezený prostor pro nohy',
  extra_legroom: 'Více místa na nohy',
  extraLegroom: 'Více místa na nohy',
  noData: 'Mapa sedadel není k dispozici',
  loading: 'Načítání mapy sedadel...',
  first: 'První třída',
  business: 'Třída Business',
  premium: 'Třída Premium',
  economy: 'Ekonomická třída',
  F: 'První třída',
  B: 'Třída Business',
  P: 'Třída Premium',
  E: 'Ekonomická třída',
  babyBassinet: 'Dětská postýlka k dispozici',
  bassinet: 'Dětská postýlka k dispozici',
  doNotRecline: 'Omezené naklápění, možné bolesti zad',
  limitedRecline: 'Omezené naklápění, možné bolesti zad',
  nearGalley: 'Blízko kuchyňky',
  nearLavatory: 'Blízko WC',
  nearStairs: 'Schody, oblast s vysokým provozem',
  noFloorStorage: 'Žádné úložné místo pod sedadlem',
  noOverheadStorage: 'Omezený úložný prostor nahoře',
  limitedOverheadStorage: 'Omezený úložný prostor nahoře',
  getColdByExit: 'Blízko k východu, průvan a chlad',
  misalignedWindow: 'Částečný nebo chybějící výhled z okna',
  noWindow: 'Částečný nebo chybějící výhled z okna',
  wingInWindow: 'Křídlo z okenního výhledu',
  trayTableInArmrest: 'Stolek v opěradle',
  storageBoxBetweenWall: 'Úložný box mezi stěnami',
  standardSeat: 'Standardní sedadlo',
  reservedCrewSeat: 'Rezervováno',
  prereclinedSeat: 'Přednakloněné sedadlo',
  personalTvInArmrest: 'Osobní TV v opěradle',
  invertedSeat: 'Sedadlo obrácené dozadu',
  narrower: 'Užší sedadlo',
  exitRow: 'Výstupní řada',
  ADT: 'dospělí',
  CHD: 'děti',
  INF: 'kojenci',
  seatRestrictions: 'Sedadlo je pouze pro',
  passenger: 'Cestující',
  passengers: 'Cestující',
  legend: 'Legenda',
  free: 'zdarma',
  gallery: 'Galerie letadla',
  allCabins: 'Všechny kabiny',
  seatsIn: 'Sedadla v',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'u okna',
  seatPositionAisle: 'u uličky',
  seatPositionMiddle: 'uprostřed',
  seatExtraLegroom: 'více místa na nohy',
  seatExitRow: 'řada nouzového východu',
  seatAvailable: 'k dispozici',
  seatUnavailable: 'nedostupné',
  seatSelected: 'vybráno',
  seatSelectedFor: 'vybráno pro',
  seatRestrictedFor: 'nedostupné pro',
  close: 'Zavřít',
  moveToSeat: 'Přejít na sedadlo',
  gridLabel: 'Plánek sedadel',
  allSeats: 'Všechna sedadla',
  row: 'Řada',
  seat: 'Sedadlo',
  cabin: 'Kabina',
  position: 'Poloha',
  features: 'Vybavení',
  price: 'Cena',
  status: 'Stav',
  action: 'Akce',
};

export const LOCALIZATION_UK: Record<string, string> = {
  deck: 'Палуба',
  cancel: 'Скасувати',
  select: 'Вибрати',
  unselect: 'Скасувати вибір',
  available: 'Доступно',
  unavailable: 'Недоступно',
  selected: 'Обрано',
  preferred: 'Бажано',
  extra: 'Додатково',
  disabled: 'Вимкнено',
  pitch: 'Відстань',
  width: 'Ширина',
  recline: 'Нахил',
  wifi: 'Wi-Fi увімкнено',
  wifi_enabled: 'Wi-Fi увімкнено',
  wifiEnabled: 'Wi-Fi увімкнено',
  power: 'Розетка',
  powerPlug: 'Розетка',
  usbPlug: 'USB-розетка',
  usbPowerPlug: 'USB і електрична розетка',
  audioVideo: 'Аудіо та відео за запитом',
  audio_video_ondemand: 'Аудіо та відео за запитом',
  entertainment: 'Аудіо та відео за запитом',
  bluetooth: 'Створіть пару з гарнітурою',
  restricted_legroom: 'Обмежене місце для ніг',
  restrictedLegroom: 'Обмежене місце для ніг',
  extra_legroom: 'Додаткове місце для ніг',
  extraLegroom: 'Додаткове місце для ніг',
  noData: 'Карта місць недоступна',
  loading: 'Завантаження карти місць...',
  first: 'Перший клас',
  business: 'Бізнес-клас',
  premium: 'Преміум-клас',
  economy: 'Економ-клас',
  F: 'Перший клас',
  B: 'Бізнес-клас',
  P: 'Преміум-клас',
  E: 'Економ-клас',
  babyBassinet: 'Ліжечко для немовляти доступне',
  bassinet: 'Ліжечко для немовляти доступне',
  doNotRecline: 'Обмежений нахил, можливі болі в спині',
  limitedRecline: 'Обмежений нахил, можливі болі в спині',
  nearGalley: 'Біля камбузу',
  nearLavatory: 'Біля туалету',
  nearStairs: 'Сходи, зона з високим рухом',
  noFloorStorage: 'Немає місця для зберігання під сидінням',
  noOverheadStorage: 'Обмежене місце для зберігання зверху',
  limitedOverheadStorage: 'Обмежене місце для зберігання зверху',
  getColdByExit: 'Біля виходу, можливі протяги та холод',
  misalignedWindow: 'Частковий або відсутній вид з вікна',
  noWindow: 'Частковий або відсутній вид з вікна',
  wingInWindow: 'Вигляд крила з вікна',
  trayTableInArmrest: 'Столик у підлокітнику',
  storageBoxBetweenWall: 'Коробка для зберігання між стінами',
  standardSeat: 'Стандартне сидіння',
  reservedCrewSeat: 'Зарезервовано',
  prereclinedSeat: 'Попередньо нахилений сидіння',
  personalTvInArmrest: 'Особистий ТБ в підлокітнику',
  invertedSeat: 'Задом наперед сидіння',
  narrower: 'Вужче сидіння',
  exitRow: 'Ряд біля виходу',
  ADT: 'дорослі',
  CHD: 'діти',
  INF: 'немовлята',
  seatRestrictions: 'Сидіння тільки для',
  passenger: 'Пасажир',
  passengers: 'Пасажири',
  legend: 'Легенда',
  free: 'безкоштовно',
  gallery: 'Галерея літака',
  allCabins: 'Усі кабіни',
  seatsIn: 'Місця в',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'біля вікна',
  seatPositionAisle: 'біля проходу',
  seatPositionMiddle: 'середнє',
  seatExtraLegroom: 'більший простір для ніг',
  seatExitRow: 'ряд аварійного виходу',
  seatAvailable: 'доступно',
  seatUnavailable: 'недоступно',
  seatSelected: 'вибрано',
  seatSelectedFor: 'вибрано для',
  seatRestrictedFor: 'недоступно для',
  close: 'Закрити',
  moveToSeat: 'Перейти до місця',
  gridLabel: 'Схема місць',
  allSeats: 'Всі місця',
  row: 'Ряд',
  seat: 'Місце',
  cabin: 'Кабіна',
  position: 'Положення',
  features: 'Зручності',
  price: 'Ціна',
  status: 'Статус',
  action: 'Дія',
};

export const LOCALIZATION_VI: Record<string, string> = {
  deck: 'Tầng',
  cancel: 'Hủy',
  select: 'Chọn',
  unselect: 'Bỏ chọn',
  available: 'Có sẵn',
  unavailable: 'Không có sẵn',
  selected: 'Đã chọn',
  preferred: 'Ưu tiên',
  extra: 'Thêm',
  disabled: 'Đã tắt',
  pitch: 'Khoảng để chân',
  width: 'Chiều rộng',
  recline: 'Ngả lưng',
  wifi: 'Có Wi‑Fi',
  wifi_enabled: 'Có Wi‑Fi',
  wifiEnabled: 'Có Wi‑Fi',
  power: 'Ổ cắm điện',
  powerPlug: 'Ổ cắm điện',
  usbPlug: 'Cổng USB',
  usbPowerPlug: 'Cổng USB và ổ cắm điện',
  audioVideo: 'Âm thanh và video theo yêu cầu',
  audio_video_ondemand: 'Âm thanh và video theo yêu cầu',
  entertainment: 'Âm thanh và video theo yêu cầu',
  bluetooth: 'Ghép đôi tai nghe',
  restricted_legroom: 'Chỗ để chân hạn chế',
  restrictedLegroom: 'Chỗ để chân hạn chế',
  extra_legroom: 'Nhiều chỗ để chân',
  extraLegroom: 'Nhiều chỗ để chân',
  noData: 'Không có bản đồ ghế',
  loading: 'Đang tải bản đồ ghế...',
  first: 'Hạng nhất',
  business: 'Hạng thương gia',
  premium: 'Hạng Premium',
  economy: 'Hạng phổ thông',
  F: 'Hạng nhất',
  B: 'Hạng thương gia',
  P: 'Hạng Premium',
  E: 'Hạng phổ thông',
  babyBassinet: 'Có nôi cho trẻ sơ sinh',
  bassinet: 'Có nôi cho trẻ sơ sinh',
  doNotRecline: 'Không được ngả lưng, có thể đau lưng',
  limitedRecline: 'Tựa lưng hạn chế, có thể đau lưng',
  nearGalley: 'Gần khu vực phục vụ ăn uống',
  nearLavatory: 'Gần nhà vệ sinh',
  nearStairs: 'Gần cầu thang, khu vực đông',
  noFloorStorage: 'Không có chỗ để đồ dưới ghế',
  noOverheadStorage: 'Không gian để hành lý trên đầu hạn chế',
  limitedOverheadStorage: 'Không gian để hành lý trên đầu hạn chế',
  getColdByExit: 'Gần cửa thoát hiểm, có gió lùa và lạnh',
  misalignedWindow: 'Cửa sổ bị lệch hoặc che khuất',
  noWindow: 'Cửa sổ bị lệch hoặc che khuất',
  wingInWindow: 'Cánh máy bay chắn tầm nhìn khỏi cửa sổ',
  trayTableInArmrest: 'Bàn ăn trong tựa tay',
  storageBoxBetweenWall: 'Hộp đựng đồ giữa tường',
  standardSeat: 'Ghế tiêu chuẩn',
  reservedCrewSeat: 'Đã dành cho phi hành đoàn',
  prereclinedSeat: 'Ghế đã được ngả sẵn',
  personalTvInArmrest: 'TV cá nhân trong tựa tay',
  invertedSeat: 'Ghế quay về phía sau',
  narrower: 'Ghế hẹp hơn',
  exitRow: 'Hàng ghế thoát hiểm',
  ADT: 'người lớn',
  CHD: 'trẻ em',
  INF: 'trẻ sơ sinh',
  seatRestrictions: 'Ghế chỉ dành cho',
  passenger: 'Hành khách',
  passengers: 'Hành khách',
  legend: 'Chú thích',
  free: 'miễn phí',
  gallery: 'Bộ sưu tập máy bay',
  allCabins: 'Tất cả khoang',
  seatsIn: 'Ghế trong',

  // WCAG a11y keys (commit 3)
  seatPositionWindow: 'cạnh cửa sổ',
  seatPositionAisle: 'cạnh lối đi',
  seatPositionMiddle: 'giữa',
  seatExtraLegroom: 'thêm chỗ để chân',
  seatExitRow: 'hàng cửa thoát hiểm',
  seatAvailable: 'còn trống',
  seatUnavailable: 'không khả dụng',
  seatSelected: 'đã chọn',
  seatSelectedFor: 'đã chọn cho',
  seatRestrictedFor: 'không khả dụng cho',
  close: 'Đóng',
  moveToSeat: 'Đến ghế',
  gridLabel: 'Sơ đồ ghế',
  allSeats: 'Tất cả ghế',
  row: 'Hàng',
  seat: 'Ghế',
  cabin: 'Khoang',
  position: 'Vị trí',
  features: 'Tính năng',
  price: 'Giá',
  status: 'Trạng thái',
  action: 'Hành động',
};

export const LOCALES_MAP: Record<string, Record<string, string>> = {
  EN: LOCALIZATION_EN,
  RU: LOCALIZATION_RU,
  CN: LOCALIZATION_CN,
  DE: LOCALIZATION_DE,
  FR: LOCALIZATION_FR,
  'FR-CA': LOCALIZATION_FR_CA,
  ES: LOCALIZATION_ES,
  IT: LOCALIZATION_IT,
  PT: LOCALIZATION_PT,
  'PT-BR': LOCALIZATION_PT_BR,
  AR: LOCALIZATION_AR,
  JA: LOCALIZATION_JA,
  KO: LOCALIZATION_KO,
  TR: LOCALIZATION_TR,
  NL: LOCALIZATION_NL,
  PL: LOCALIZATION_PL,
  CS: LOCALIZATION_CS,
  UK: LOCALIZATION_UK,
  VI: LOCALIZATION_VI,
};

export const FEATURE_ICONS: Record<string, string> = {
  wifi: 'wifi',
  wifienabled: 'wifi',
  power: 'power',
  poweroutlet: 'power',
  usb: 'usb',
  usbport: 'usb',
  audiovideo: 'audioVideo',
  bluetooth: 'bluetooth',
};

/** Inline SVG icons for seat features, keyed by feature name */
export const SEAT_FEATURES_ICONS: Record<string, string> = {
  '+': '<svg width="20" height="20" viewBox="-1 -1 22 22" xmlns="http://www.w3.org/2000/svg"><path d="M10 0C4.48 0 0 4.48 0 10C0 15.52 4.48 20 10 20C15.52 20 20 15.52 20 10C20 4.48 15.52 0 10 0ZM7.29 14.29L3.7 10.7C3.31 10.31 3.31 9.68 3.7 9.29C4.09 8.9 4.72 8.9 5.11 9.29L8 12.17L14.88 5.29C15.27 4.9 15.9 4.9 16.29 5.29C16.68 5.68 16.68 6.31 16.29 6.7L8.7 14.29C8.32 14.68 7.68 14.68 7.29 14.29Z" fill="#11d900"></path></svg>',
  '-': '<svg width="20" height="20" viewBox="-1 -1 22 22" xmlns="http://www.w3.org/2000/svg"><path d="M11.89 6.7L10 8.59L8.11 6.7C7.72 6.31 7.09 6.31 6.7 6.7C6.31 7.09 6.31 7.72 6.7 8.11L8.59 10L6.7 11.89C6.31 12.28 6.31 12.91 6.7 13.3C7.09 13.69 7.72 13.69 8.11 13.3L10 11.41L11.89 13.3C12.28 13.69 12.91 13.69 13.3 13.3C13.69 12.91 13.69 12.28 13.3 11.89L11.41 10L13.3 8.11C13.69 7.72 13.69 7.09 13.3 6.7C12.91 6.32 12.27 6.32 11.89 6.7ZM10 0C4.47 0 0 4.47 0 10C0 15.53 4.47 20 10 20C15.53 20 20 15.53 20 10C20 4.47 15.53 0 10 0ZM10 18C5.59 18 2 14.41 2 10C2 5.59 5.59 2 10 2C14.41 2 18 5.59 18 10C18 14.41 14.41 18 10 18Z" fill="red"></path></svg>',
  wifi: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15.23 11"><path d="M6.62,9.91A1.08,1.08,0,1,0,7.7,8.82,1.08,1.08,0,0,0,6.62,9.91M7.7,7.64A2.24,2.24,0,0,1,9.84,9.26L11,8a3.76,3.76,0,0,0-6.65.09L5.54,9.32A2.25,2.25,0,0,1,7.7,7.64m0-3.06a5.25,5.25,0,0,1,4.37,2.35l1.08-1.15a6.75,6.75,0,0,0-11,.14L3.25,7A5.26,5.26,0,0,1,7.69,4.58m0-3a8.19,8.19,0,0,1,6.45,3.15l1.08-1.14A9.73,9.73,0,0,0,0,3.78L1.11,4.89A8.22,8.22,0,0,1,7.7,1.56" fill="#4f6f8f"/></svg>',
  wifi_enabled:
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15.23 11"><path d="M6.62,9.91A1.08,1.08,0,1,0,7.7,8.82,1.08,1.08,0,0,0,6.62,9.91M7.7,7.64A2.24,2.24,0,0,1,9.84,9.26L11,8a3.76,3.76,0,0,0-6.65.09L5.54,9.32A2.25,2.25,0,0,1,7.7,7.64m0-3.06a5.25,5.25,0,0,1,4.37,2.35l1.08-1.15a6.75,6.75,0,0,0-11,.14L3.25,7A5.26,5.26,0,0,1,7.69,4.58m0-3a8.19,8.19,0,0,1,6.45,3.15l1.08-1.14A9.73,9.73,0,0,0,0,3.78L1.11,4.89A8.22,8.22,0,0,1,7.7,1.56" fill="#4f6f8f"/></svg>',
  wifiEnabled:
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15.23 11"><path d="M6.62,9.91A1.08,1.08,0,1,0,7.7,8.82,1.08,1.08,0,0,0,6.62,9.91M7.7,7.64A2.24,2.24,0,0,1,9.84,9.26L11,8a3.76,3.76,0,0,0-6.65.09L5.54,9.32A2.25,2.25,0,0,1,7.7,7.64m0-3.06a5.25,5.25,0,0,1,4.37,2.35l1.08-1.15a6.75,6.75,0,0,0-11,.14L3.25,7A5.26,5.26,0,0,1,7.69,4.58m0-3a8.19,8.19,0,0,1,6.45,3.15l1.08-1.14A9.73,9.73,0,0,0,0,3.78L1.11,4.89A8.22,8.22,0,0,1,7.7,1.56" fill="#4f6f8f"/></svg>',
  power:
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 21"><path d="M20.07,10.16a1.34,1.34,0,0,1-1.34,1.34H3.5a1.34,1.34,0,1,1,0-2.68h2V1.12a1.12,1.12,0,1,1,2.24,0v7.7h6.73V1.12a1.12,1.12,0,1,1,2.24,0v7.7h2A1.34,1.34,0,0,1,20.07,10.16ZM3.54,12.63V18.8A2.25,2.25,0,0,0,5.4,21H16.84a2.24,2.24,0,0,0,1.85-2.2V12.62Z" fill="#4f6f8f"></path></svg>',
  audioVideo:
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 21"><path d="M6.51,8.59h3.57L8.44,11.13H4.88ZM18.45,2.31,17.43,0,15,1.08l2.53,1.65Zm-3.16,1.4L12.77,2.07,9.51,3.52,12,5.17Zm-4.42,7.41h3.57l1.64-2.53H12.51Zm-1.05-5L7.29,4.51,4,6,6.56,7.6ZM18.5,8.59l-1.64,2.53h2.67V8.59Zm-16,11.24A1.16,1.16,0,0,0,3.63,21H18.37a1.16,1.16,0,0,0,1.16-1.16V13.2H2.47Zm.17-13V11.2L4.51,8.41Z" fill="#4f6f8f"></path></svg>',
  bluetooth:
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 148 148"><g transform="translate(0,148) scale(0.1,-0.1)" fill="#4f6f8f" stroke="none"><path d="M667 1459 l-27 -20 -2 -264 -3 -264 -117 115 c-96 94 -122 114 -147 114 -31 0 -71 -38 -71 -68 0 -9 71 -87 157 -174 l157 -158 -157 -158 c-86 -87 -157 -165 -157 -174 0 -30 40 -68 72 -68 25 0 50 20 149 117 l119 116 0 -266 0 -266 26 -20 c15 -12 35 -21 45 -21 21 0 431 338 455 375 9 14 13 32 9 45 -4 11 -85 86 -181 166 -96 80 -174 150 -174 154 0 4 78 74 174 154 96 80 177 155 181 166 4 13 0 31 -9 45 -24 37 -434 375 -455 375 -10 0 -30 -9 -44 -21z m229 -289 c56 -47 103 -88 103 -92 1 -7 -204 -181 -219 -186 -6 -2 -10 68 -10 183 0 132 3 186 11 183 6 -2 58 -42 115 -88z m-1 -673 c58 -49 105 -92 104 -95 0 -8 -203 -175 -218 -180 -8 -3 -11 51 -11 183 0 115 4 185 10 183 5 -1 57 -43 115 -91z"/></g></svg>',
  dot: '<svg width="20" height="20" viewBox="-1 -1 22 22" xmlns="http://www.w3.org/2000/svg"><path d="M10 0C4.48 0 0 4.48 0 10C0 15.52 4.48 20 10 20C15.52 20 20 15.52 20 10C20 4.48 15.52 0 10 0Z" fill="#4f6f8f"></path></svg>',
};

/** Inline SVG icons for seat measurements (pitch, width, recline) */
export const SEAT_MEASUREMENTS_ICONS: Record<string, string> = {
  recline:
    '<svg width="48" height="51" viewBox="0 0 48 51" xmlns="http://www.w3.org/2000/svg"><g transform="scale(-1, 1) translate(-48,0)"><path d="M19.3954 30.8342C19.7952 31.1919 20.4265 30.8973 20.4265 30.3502C20.4265 30.1818 20.3424 30.0345 20.2161 29.9082L20.1319 29.8241L19.2902 29.1507L19.2691 29.1296C18.8272 28.7298 18.1538 29.1086 18.259 29.6978C18.2801 29.8451 18.3432 29.9503 18.4484 30.0345L18.4695 30.0555L19.3112 30.7289L19.3954 30.8131V30.8342Z"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M14.5973 0.152473C13.2084 0.510215 12.051 1.49927 11.8616 3.2038V3.30902C11.4284 12.9854 11.4976 17.6783 12.4905 21.7889L11.967 21.091L11.946 21.0489C11.8197 20.8384 11.5672 20.7543 11.3357 20.7753C10.8938 20.8384 10.6833 21.3435 10.9148 21.7223L10.9359 21.7643L11.6303 22.6903L11.6724 22.7534L11.6934 22.7955C11.9769 23.1853 12.5288 23.0977 12.734 22.7212C12.8634 23.1823 13.0052 23.6402 13.1601 24.1013C13.1537 24.128 13.1488 24.1557 13.1454 24.1844C13.1244 24.3527 13.1875 24.5211 13.2927 24.6473L13.3138 24.6894L13.4099 24.8175C14.1463 26.8561 15.1378 29.0219 16.4364 31.8585L16.4492 31.8863C16.5014 32.0005 16.5542 32.1159 16.6075 32.2325C17.1074 33.3248 17.6558 34.5232 18.246 35.841C18.0278 35.9337 17.8592 36.149 17.8592 36.4318C17.8592 36.6001 17.9434 36.7474 18.0697 36.8737L18.8904 37.6944L18.9535 37.7365C19.0105 37.7891 19.073 37.828 19.1381 37.8543C19.2985 38.2206 19.4614 38.5951 19.6267 38.978C19.9424 39.7355 20.2581 40.2616 20.9735 40.6615C21.5838 40.9982 22.3624 41.1455 23.625 41.1665C24.8703 41.187 26.0681 41.2 27.2319 41.2077L29.5644 48.0056H22.8676C22.1942 48.0056 21.626 48.5527 21.626 49.2472C21.626 49.9206 22.1731 50.4888 22.8676 50.4888H31.1617C31.2508 50.4988 31.3418 50.499 31.433 50.4888H42.838C43.5114 50.4888 44.0795 49.9416 44.0795 49.2472C44.0795 48.5738 43.5324 48.0056 42.838 48.0056H32.1957L29.8663 41.2168C31.8256 41.2179 33.7121 41.2077 35.5976 41.1974C36.2285 41.194 36.8594 41.1906 37.4928 41.1876C38.3328 41.1876 39.1769 41.1835 40.1246 41.179C41.3371 41.1731 42.7192 41.1665 44.4793 41.1665C45.7419 41.1876 46.5836 40.3037 47.0045 39.1253C47.1518 38.7044 47.236 38.2415 47.257 37.7785C47.2781 37.3155 47.236 36.8315 47.1308 36.3896C46.8151 35.148 46.0155 34.1379 44.5845 34.0117C40.2916 33.6539 37.4297 33.4014 34.5888 33.1489C33.2553 33.0322 31.925 32.9122 30.6452 32.7968C28.6936 32.6208 26.859 32.4553 25.3085 32.3282C19.7951 20.3544 18.6166 17.5976 19.6057 3.43528C19.7109 1.96223 18.8481 0.910044 17.6486 0.383953C17.1857 0.173516 16.6806 0.0472546 16.1545 0.00516733C15.6285 -0.0158763 15.1024 0.026211 14.5973 0.152473ZM28.3161 38.6924C31.509 38.7099 34.4611 38.691 37.4717 38.6623C40.0812 38.6413 42.7116 38.6202 44.4582 38.6202H44.4793C44.4793 38.6202 44.5424 38.4729 44.6266 38.2625C44.6897 38.0731 44.7318 37.8416 44.7528 37.6101C44.7739 37.3787 44.7528 37.1472 44.6897 36.9367C44.6266 36.6632 44.5214 36.4738 44.353 36.4527C43.8105 36.4054 43.2248 36.3545 42.6047 36.3006C40.1945 36.0912 37.2654 35.8367 34.3362 35.5689L34.335 35.5688C31.4945 35.3163 28.6538 35.0638 24.3405 34.7061L23.625 34.643L23.3304 33.9906L23.2462 33.8223C23.1555 33.6256 23.0658 33.4313 22.9772 33.2392C22.7687 33.4555 22.4052 33.5083 22.1311 33.2752V33.2963L22.0469 33.2331L21.5419 32.8123L21.2052 32.4756C21.0789 32.3493 21.0158 32.223 20.9947 32.0547C20.9526 31.5075 21.605 31.1919 22.0048 31.5496L22.3415 31.8863L22.3617 31.9031C17.1874 20.6464 16.1038 17.4332 17.1015 3.24589C17.1226 2.97232 16.9121 2.76188 16.6175 2.63562C16.4281 2.55145 16.1966 2.48832 15.9441 2.46727C15.6916 2.44623 15.4391 2.46727 15.2076 2.5304L15.2286 2.55145C14.7867 2.67771 14.4079 2.97232 14.3448 3.45632C13.7423 17.155 14.1695 20.4518 16.9083 26.8319L17.5225 27.4461C17.7771 27.7007 17.767 28.0756 17.5559 28.3069C17.9026 29.0826 18.2795 29.9097 18.6886 30.8073L18.7008 30.8341C19.6057 32.7912 20.6579 35.0849 21.8995 37.9679C22.0047 38.2415 22.1099 38.4098 22.1941 38.4519C22.4045 38.5781 22.8254 38.6202 23.6461 38.6413C25.1362 38.6658 26.5566 38.6814 27.9307 38.6901C28.0607 38.6701 28.1906 38.6716 28.3161 38.6924Z"></path><path d="M0.140476 10.527L0.132009 10.5185L0.182563 10.6533L0.0983887 10.485V9.05399C0.140476 8.88564 0.245694 8.75938 0.371956 8.67521C0.834916 8.38059 1.42414 8.80147 1.29788 9.32756V10.1062L1.38205 10.3587C1.5504 10.7796 1.2137 11.2215 0.771785 11.2004C0.540305 11.1794 0.329869 11.0321 0.245694 10.8006L0.22465 10.7375L0.140476 10.527Z"></path><path d="M2.26588 12.6525C2.05545 12.1264 1.31892 12.1474 1.12953 12.6525C1.0664 12.7998 1.0664 12.9471 1.12953 13.0944L1.15057 13.1575L1.48727 13.9151L1.59249 14.2307C1.80292 14.7358 2.51841 14.7358 2.72884 14.2307C2.79197 14.0834 2.79197 13.9151 2.72884 13.7678L2.56049 13.2627L2.30797 12.6945L2.28693 12.6314L2.26588 12.6525Z"></path><path d="M3.63372 16.0405C3.42328 15.5354 2.7078 15.5354 2.49736 16.0405C2.43423 16.1878 2.43423 16.3561 2.49736 16.5034V16.5245L2.68676 17.0295L2.93928 17.6188V17.6398C3.17076 18.1659 3.90729 18.1238 4.07564 17.5977C4.11772 17.4504 4.11772 17.3031 4.05459 17.1558V17.1347L3.73894 16.3982L3.61268 16.0615V16.0405H3.63372Z"></path><path d="M5.08573 19.3864C4.83321 18.8393 4.05459 18.9445 3.92833 19.5337C3.90729 19.66 3.92833 19.7862 3.97042 19.9125V19.9336L4.51755 21.0278C4.62277 21.2383 4.85425 21.3856 5.08573 21.3856C5.52765 21.3645 5.80121 20.9016 5.61182 20.5017L5.06469 19.4075V19.3864H5.08573Z"></path><path d="M5.56973 22.7534C5.5066 22.9217 5.52765 23.0901 5.61182 23.2374L6.24313 24.2896L6.28522 24.3527C6.41148 24.5421 6.62192 24.6684 6.85339 24.6473C7.31636 24.6263 7.56888 24.1212 7.3374 23.7214L7.29531 23.6583L6.664 22.6061L6.68505 22.6482C6.43252 22.1852 5.73808 22.2273 5.56973 22.7534Z"></path><path d="M8.57897 25.6995C8.26332 25.2576 7.54784 25.4259 7.46366 25.9731C7.44262 26.1204 7.4847 26.2887 7.56888 26.415L8.32645 27.4251L8.34749 27.4461C8.68419 27.867 9.37863 27.6776 9.44176 27.1305C9.46281 26.9621 9.42072 26.8148 9.3155 26.6886L8.55793 25.6785L8.57897 25.6995Z"></path><path d="M10.8517 28.4773C10.7044 28.3089 10.4729 28.2458 10.2625 28.3089C9.7995 28.4141 9.63116 28.9823 9.96785 29.319L9.9889 29.3401L10.3677 29.8451L10.8306 30.3081C11.2094 30.6868 11.8828 30.4133 11.8828 29.8661C11.8828 29.6978 11.8197 29.5505 11.6934 29.4453L11.6513 29.4032L11.5251 29.2348L10.9359 28.6456L10.8517 28.4773Z"></path><path d="M13.4401 31.0235C13.0613 30.6658 12.451 30.9183 12.4089 31.4444C12.4089 31.6338 12.472 31.8022 12.5983 31.9284L13.4401 32.7702C13.8399 33.1279 14.4712 32.8333 14.4712 32.3072C14.4712 32.1389 14.387 31.9705 14.2608 31.8653L13.419 31.0235H13.4401Z"></path><path d="M16.1547 33.4857C15.7549 33.1279 15.1446 33.4225 15.1446 33.9486C15.1446 34.117 15.2288 34.2853 15.355 34.4116L16.2178 35.2744C16.3651 35.4217 16.5756 35.4638 16.786 35.4217C17.27 35.2954 17.4173 34.7062 17.0385 34.3695L16.1547 33.4857Z"></path><path d="M9.67324 19.6389C9.94681 20.1019 10.6202 20.0388 10.7886 19.5127C10.8306 19.3443 10.8306 19.176 10.7465 19.0287L10.3887 18.3132L10.3046 18.1028L10.2625 18.0186C10.031 17.5556 9.33654 17.5977 9.14715 18.0817C9.08402 18.2501 9.08402 18.4184 9.16819 18.5868L9.21028 18.6709L9.54698 19.5127L9.6522 19.7231L9.67324 19.6389Z"></path><path d="M8.0108 16.314C8.11601 16.5455 8.32645 16.6718 8.57897 16.6718C9.02089 16.6507 9.29446 16.2088 9.12611 15.809L8.78941 14.9673L8.74732 14.841L8.70523 14.7568C8.4948 14.2728 7.8214 14.2518 7.58992 14.7358C7.52679 14.9041 7.50575 15.0725 7.58992 15.2408L7.63201 15.325L7.80036 15.83L8.0108 16.3561V16.314Z"></path><path d="M6.57983 12.905C6.79026 13.4311 7.54784 13.41 7.73723 12.8839C7.77931 12.7366 7.77931 12.5893 7.73723 12.4631L7.71618 12.442L7.54784 11.937L7.4847 11.8107L7.29531 11.3688V11.3477C7.08488 10.8006 6.30626 10.8217 6.13791 11.3898C6.09582 11.5161 6.09582 11.6424 6.15896 11.7897V11.8107L6.47461 12.5472L6.664 12.926L6.68505 12.9471L6.57983 12.905Z"></path><path d="M5.27512 9.45382C5.48556 10.001 6.26417 9.97991 6.43252 9.43278C6.47461 9.30651 6.47461 9.18025 6.43252 9.05399L6.41148 8.99086L6.28522 8.69625L6.03269 8.02285L5.94852 7.8545L5.92748 7.83346C5.696 7.32841 4.95947 7.3705 4.79112 7.89659C4.74903 8.0439 4.74903 8.1912 4.81216 8.33851V8.44372L5.12782 9.28547L5.19095 9.43278L5.21199 9.49591L5.27512 9.45382Z"></path><path d="M3.44433 7.66511C3.97042 7.62302 4.2019 6.97067 3.82311 6.61293C3.69685 6.48667 3.5285 6.44458 3.36015 6.44458H2.28693L1.65562 6.76023H1.63457C1.40309 6.86545 1.27683 7.09693 1.27683 7.32841C1.29788 7.77033 1.73979 8.0439 2.13962 7.8545H3.00241L3.44433 7.66511Z"></path></g></svg>',
  width:
    '<svg width="35" height="50" viewBox="0 0 35 50" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M31.1151 33.3111C32.0089 33.4715 32.8177 33.9081 33.4335 34.5238C34.2198 35.31 34.7141 36.4106 34.7141 37.6011C34.7141 38.7916 34.2198 39.8922 33.4335 40.6784C32.6472 41.4645 31.5464 41.9587 30.3557 41.9587H23.2082L23.1928 47.4843H27.8396C28.5361 47.4843 29.0977 48.0458 29.0977 48.7421C29.0977 49.4385 28.5361 50 27.8396 50H7.86769C7.17126 50 6.60962 49.4385 6.60962 48.7421C6.60962 48.0458 7.17126 47.4843 7.86769 47.4843H12.4992L12.5146 41.9587H5.35149C4.16082 41.9587 3.06 41.4645 2.2737 40.6784C1.48741 39.8922 0.993164 38.7916 0.993164 37.6011C0.993164 36.4106 1.48741 35.31 2.2737 34.5238C2.88437 33.9133 3.68474 33.4788 4.56958 33.3152L4.82779 23.4241C4.72932 23.4271 4.63334 23.4314 4.51247 23.4369C4.3114 23.446 4.04146 23.4581 3.57663 23.4727C2.90266 23.4951 2.31855 22.956 2.29609 22.2597C2.27362 21.5859 2.8128 21.0019 3.50923 20.9794C3.98488 20.965 4.26402 20.9529 4.47273 20.9438C4.63574 20.9368 4.75578 20.9316 4.89293 20.9286L5.32902 4.22282C5.35149 3.05481 5.84573 1.9991 6.60956 1.2354C7.37339 0.471698 8.42928 0 9.59749 0H26.0873C27.2555 0 28.3114 0.471698 29.0752 1.2354C29.839 1.9991 30.3108 3.05481 30.3557 4.22282L30.7921 20.9384C30.8322 20.94 30.8743 20.9419 30.9198 20.9438C31.1286 20.9529 31.4077 20.965 31.8833 20.9794C32.5798 21.0019 33.1189 21.5859 33.0965 22.2597C33.074 22.956 32.4899 23.4951 31.8159 23.4727C31.3511 23.4581 31.0812 23.446 30.8801 23.4369C30.8724 23.4365 30.8648 23.4362 30.8573 23.4359L31.1151 33.3111ZM20.6991 47.4843L20.7145 41.9587H15.0083L14.9929 47.4843H20.6991ZM27.8171 4.26775L28.5809 33.221H7.05887L7.82271 4.26775C7.82271 3.77359 8.0249 3.32435 8.36188 2.98742C8.6764 2.67296 9.10325 2.4708 9.57502 2.4708H26.0648C26.5366 2.4708 26.9634 2.67296 27.2779 2.98742C27.5925 3.32435 27.7946 3.77359 27.8171 4.26775ZM30.3557 35.7368H5.35149C4.83479 35.7368 4.38547 35.9389 4.04849 36.2759C3.7115 36.6128 3.50931 37.0845 3.50931 37.5787C3.50931 38.0953 3.7115 38.5445 4.04849 38.8814C4.38547 39.2184 4.85725 39.4205 5.35149 39.4205H30.3557C30.8724 39.4205 31.3218 39.2184 31.6587 38.8814C31.9957 38.5445 32.1979 38.0728 32.1979 37.5787C32.1979 37.062 31.9957 36.6128 31.6587 36.2759C31.3218 35.9389 30.85 35.7368 30.3557 35.7368Z"></path></svg>',
  pitch:
    '<svg width="74" height="51" viewBox="0 0 74 51" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M2.98892 0.152473C1.60004 0.510215 0.442636 1.49927 0.253243 3.2038V3.30902C-0.503624 20.2124 0.272842 21.9085 4.82805 31.8585L4.84076 31.8863L4.99918 32.2325C5.87007 34.1358 6.88816 36.3607 8.01834 38.978C8.334 39.7355 8.64965 40.2616 9.36514 40.6615C9.9754 40.9982 10.754 41.1455 12.0166 41.1665C13.2619 41.187 14.4597 41.2 15.6234 41.2077L17.9559 48.0057H11.2591C10.5857 48.0057 10.0175 48.5528 10.0175 49.2473C10.0175 49.9207 10.5646 50.4888 11.2591 50.4888H19.5541C19.6426 50.4987 19.7329 50.499 19.8235 50.4888H31.2295C31.9029 50.4888 32.471 49.9417 32.471 49.2473C32.471 48.5739 31.9239 48.0057 31.2295 48.0057H20.5872L18.2578 41.2168C20.2165 41.2179 22.1025 41.2077 23.9874 41.1974C24.6238 41.194 25.2528 41.1906 25.8844 41.1876C26.7241 41.1876 27.5679 41.1835 28.5151 41.179C29.7287 41.1731 31.1108 41.1665 32.8709 41.1665C34.1335 41.1876 34.9752 40.3037 35.3961 39.1253C35.5434 38.7044 35.6276 38.2415 35.6486 37.7785C35.6697 37.3155 35.6276 36.8315 35.5224 36.3896C35.2067 35.148 34.4071 34.1379 32.9761 34.0117C28.6832 33.6539 25.8213 33.4014 22.9804 33.1489C21.6481 33.0323 20.3191 32.9124 19.0403 32.7971C17.085 32.6207 15.2506 32.4553 13.7001 32.3282C8.18669 20.3544 7.00825 17.5976 7.9973 3.43528C8.10252 1.96223 7.23973 0.910044 6.04024 0.383953C5.57728 0.173516 5.07224 0.0472546 4.54614 0.00516733C4.02005 -0.0158763 3.49396 0.026211 2.98892 0.152473ZM16.7077 38.6924C19.9006 38.7099 22.8527 38.691 25.8633 38.6623C28.4728 38.6413 31.1032 38.6202 32.8498 38.6202H32.8709C32.8709 38.6202 32.934 38.4729 33.0182 38.2625C33.0813 38.0731 33.1234 37.8416 33.1444 37.6101C33.1655 37.3787 33.1444 37.1472 33.0813 36.9367C33.0182 36.6632 32.913 36.4738 32.7446 36.4527C32.2021 36.4054 31.6163 36.3545 30.9963 36.3006C28.5853 36.0911 25.6566 35.8367 22.7278 35.5689C19.8846 35.3162 17.0443 35.0637 12.7321 34.7061L12.0166 34.643L11.722 33.9906L11.6378 33.8223C5.70355 20.9646 4.44093 18.2079 5.49311 3.24589C5.51415 2.97232 5.30371 2.76188 5.0091 2.63562C4.81971 2.55145 4.58823 2.48832 4.33571 2.46727C4.08319 2.44623 3.83066 2.46727 3.59918 2.5304L3.62022 2.55145C3.17831 2.67771 2.79952 2.97232 2.73639 3.45632C2.02158 19.7078 2.75606 21.3194 7.08023 30.8073L7.09242 30.8341C7.9973 32.7911 9.04948 35.0849 10.2911 37.9679C10.3963 38.2415 10.5015 38.4098 10.5857 38.4519C10.7961 38.5781 11.217 38.6202 12.0377 38.6413C13.5277 38.6658 14.948 38.6814 16.322 38.6901C16.4521 38.67 16.5821 38.6715 16.7077 38.6924Z"/><g transform="translate(38,0)"><path fill-rule="evenodd" clip-rule="evenodd" d="M2.98892 0.152473C1.60004 0.510215 0.442636 1.49927 0.253243 3.2038V3.30902C-0.503624 20.2124 0.272842 21.9085 4.82805 31.8585L4.84076 31.8863L4.99918 32.2325C5.87007 34.1358 6.88816 36.3607 8.01834 38.978C8.334 39.7355 8.64965 40.2616 9.36514 40.6615C9.9754 40.9982 10.754 41.1455 12.0166 41.1665C13.2619 41.187 14.4597 41.2 15.6234 41.2077L17.9559 48.0057H11.2591C10.5857 48.0057 10.0175 48.5528 10.0175 49.2473C10.0175 49.9207 10.5646 50.4888 11.2591 50.4888H19.5541C19.6426 50.4987 19.7329 50.499 19.8235 50.4888H31.2295C31.9029 50.4888 32.471 49.9417 32.471 49.2473C32.471 48.5739 31.9239 48.0057 31.2295 48.0057H20.5872L18.2578 41.2168C20.2165 41.2179 22.1025 41.2077 23.9874 41.1974C24.6238 41.194 25.2528 41.1906 25.8844 41.1876C26.7241 41.1876 27.5679 41.1835 28.5151 41.179C29.7287 41.1731 31.1108 41.1665 32.8709 41.1665C34.1335 41.1876 34.9752 40.3037 35.3961 39.1253C35.5434 38.7044 35.6276 38.2415 35.6486 37.7785C35.6697 37.3155 35.6276 36.8315 35.5224 36.3896C35.2067 35.148 34.4071 34.1379 32.9761 34.0117C28.6832 33.6539 25.8213 33.4014 22.9804 33.1489C21.6481 33.0323 20.3191 32.9124 19.0403 32.7971C17.085 32.6207 15.2506 32.4553 13.7001 32.3282C8.18669 20.3544 7.00825 17.5976 7.9973 3.43528C8.10252 1.96223 7.23973 0.910044 6.04024 0.383953C5.57728 0.173516 5.07224 0.0472546 4.54614 0.00516733C4.02005 -0.0158763 3.49396 0.026211 2.98892 0.152473ZM16.7077 38.6924C19.9006 38.7099 22.8527 38.691 25.8633 38.6623C28.4728 38.6413 31.1032 38.6202 32.8498 38.6202H32.8709C32.8709 38.6202 32.934 38.4729 33.0182 38.2625C33.0813 38.0731 33.1234 37.8416 33.1444 37.6101C33.1655 37.3787 33.1444 37.1472 33.0813 36.9367C33.0182 36.6632 32.913 36.4738 32.7446 36.4527C32.2021 36.4054 31.6163 36.3545 30.9963 36.3006C28.5853 36.0911 25.6566 35.8367 22.7278 35.5689C19.8846 35.3162 17.0443 35.0637 12.7321 34.7061L12.0166 34.643L11.722 33.9906L11.6378 33.8223C5.70355 20.9646 4.44093 18.2079 5.49311 3.24589C5.51415 2.97232 5.30371 2.76188 5.0091 2.63562C4.81971 2.55145 4.58823 2.48832 4.33571 2.46727C4.08319 2.44623 3.83066 2.46727 3.59918 2.5304L3.62022 2.55145C3.17831 2.67771 2.79952 2.97232 2.73639 3.45632C2.02158 19.7078 2.75606 21.3194 7.08023 30.8073L7.09242 30.8341C7.9973 32.7911 9.04948 35.0849 10.2911 37.9679C10.3963 38.2415 10.5015 38.4098 10.5857 38.4519C10.7961 38.5781 11.217 38.6202 12.0377 38.6413C13.5277 38.6658 14.948 38.6814 16.322 38.6901C16.4521 38.67 16.5821 38.6715 16.7077 38.6924Z"/></g></svg>',
};

/** Letters used for seat columns */
export const SEAT_LETTERS = 'ABCDEFGHJKLMNOPQRSTUVWXYZ';
