...
- **2026-06-18 — «Skip to seat selection» link — добавлен и затем ОТМЕНЁН по решению пользователя.** Сначала добавили комплементарную skip-ссылку (фокус на roving-якорь), но пользователь решил, что она не нужна — `jumpToSeatsLabel` / `onJumpToSeatsClick` / шаблонный блок удалены. Осталась только дефолтная «Skip seat map» (skip-over). Механика Tab→грид→стрелки и без неё работает (см. отдельный фикс skip-link visibility ниже). Вывод на будущее: к гриду keyboard-only добираются обычными табами; отдельная «войти в грид» ссылка избыточна.
- **2026-06-18 — Skip-link visibility (clip вместо `top:-40px`):** «скрытие» skip-link через `top:-40px` не работало — регион в середине страницы, ссылки оставались видимыми над картой (и наезжали друг на друга, пока их было две). Переведено на clip/sr-only: скрыто до фокуса, на фокусе виден один аккуратный бокс. Проверено в браузере (base 1×1, focus 166×33).
